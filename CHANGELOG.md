...
- Spend never asks (2026-09-04, from the same `no-coi-sandbox-tier` run): `STOP-1c` (budget exhausted after the re-cut) and `STOP-1d` (slice cap) are removed from the closed stop list — a stop must name what the user decides that the agent cannot, and "may I keep working?" is a status line, not a decision. The budget becomes a tripwire that forces the next rung of an agent-owned ladder — re-cut (`STOP-4`) → park the unit (`status: draft` + ledger line, take the next frontier child) → run ends with the `STOP-6` report — and it counts churn, not work: a round spends the count only when its reception yields no `FIX` (`REV-12`) or a blocker survives unchanged. Overturns `STOP-3`'s "new findings each round are not progress either" (written against a reviewer inventing exactness demands, it also punished one finding real bugs). Evidence: a Final whose adjudication ruled 5/5 HOLDS — five real defects inside frozen `I4`/`I6`/`I8`/`I10` — stopped the run for 4.5 h with three identical re-audits, because the round counter was spent; the user's answer could only ever be "continue". `.claude/workflows/goal-run.js` walks the same ladder (`kind: 'parked'` / `'parked-out'` / `'slice-cap'` reports, parked units excluded from the frontier) instead of returning `STOP-1c`.
- Review reception (2026-09-04, from the 2026-09-03 `no-coi-sandbox-tier` run): every finding gets one disposition before any fix — FIX (HOLDS), REJECT (STRETCH/FALSE → `rejected:` ledger line, never re-raised), NOTE (concern / `weak` → `note:` line) — and a finding never becomes a unit, fog line or split seed on agent authority; a NOTE graduates only via `rifty-refine` (`review.md` `REV-12`, `checkpoint-run.md` §3, `rechart.md` 3, `stops.md` `STOP-4` 2). Trace validity moves to compile: a row is traced only when the target clause states its property (`readiness.md` `RDY-3`). Overturns "concerns batched to backlog at the agent's choice" and RECHART's "required discovery → draft child" for review findings: the 09-02 build-loop Final (15 blockers) was split into four proof-HOLD children and cost 8 h, three proof-only slices, 0 product lines. Grafted from obra/superpowers `receiving-code-review` (verify before acting, reasoned pushback, no performative agreement) and Spec Kit `analyze` (route a finding to the step that owns it); BMAD/OpenSpec/Kiro carry no review-reception step.
