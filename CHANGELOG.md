...
- **All 10 `@riftydev/*` libraries (+ `@riftydev/shadow-registry`) are now publishable to npm (ADR-0070).** Each package gains a `tsup` build (ESM + bundled `.d.ts` in `dist/`), a `publishConfig` that points the published `main`/`module`/`types`/`exports` at `dist/` while the in-repo `exports` keep pointing at raw `./src/*.ts` (dev/HMR loop unchanged), plus `version`/`license`/`repository`/`keywords`/`sideEffects`/`files`. `private` dropped. Source of truth: `tools/publishing/sync-publish-config.mjs` (`pnpm sync:publish`); release on a `v*` tag via `.github/workflows/release.yml`. `@riftydev/runtime-wasi` gains a `./worker-entry` subpath; `@riftydev/runtime-js` drops the unused `acorn-walk` dep. Verified by packing all 11 and importing them from a clean npm consumer. See `docs/PUBLISHING.md`.
- **Umbrella `rifty` package — one-install front door (ADR-0071, EPIC B).** A 12th publishable package (unscoped `rifty`): subpath re-exports of every layer (`rifty/vfs`, `rifty/io`, `rifty/kernel`, `rifty/runtime`, `rifty/wasi`, `rifty/net`, `rifty/npm-client`, `rifty/shell`, `rifty/terminal`, `rifty/service-worker`, kept external so singletons stay shared), a framework-free `createSandbox()` boot façade, and `checkCapabilities()`. The workspace root package is renamed `rifty` → `rifty-workspace` to free the name.
- **Releases are now tokenless via npm OIDC trusted publishing.** `release.yml` drops the `NPM_TOKEN` secret entirely and publishes with `id-token: write` + provenance; `packageManager` pinned to `pnpm@11.5.1` (11.0.x 404s on OIDC). First publish of each new name still needs a one-time token (npm can't attach trust to a package that doesn't exist yet); every release after is tokenless. See `docs/PUBLISHING.md`.
