...
- **Cold npm-install speedup backlog — 1 epic (ready) + 6 items (1 ready, 5 draft).** New `docs/backlog/epics/cold-npm-install-speedup`: make a cold, no-lockfile `npm install` finish fast without trading fidelity (resolved tree stays byte-identical to real npm) or the `walkAndPin` placement determinism invariant. Flagship ready item `npm-client/abbreviated-packuments` (corgi `Accept` header → ~10-20x smaller metadata, parity-gated); a `perf/cold-install-metadata-reprofile` decision gate (bytes-vs-RTT, on the existing `perf/cold-start-and-install-benchmark` harness) then selects among draft levers `npm-client/server-side-closure-resolver` (ADR-gated ceiling), `npm-client/bundled-popular-subgraph-metadata`, `npm-client/persisted-packument-store`, and `perf/install-transport-tuning`. Rejected levers (global OPFS CAS, ETag/304, brotli, worker-offload decompress, OPFS write consolidation, streaming SRI) recorded as out-of-scope. Sourced from a 2026-06-27 deep-research pass over `@riftydev/npm-client`.
