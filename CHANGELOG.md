...
- Process shakedown banked (2026-08-31, `docs/process/shakedown.md`): 12 rehearsal cells, each on a real merged-PR or live-backlog seed, walked through the process as written; 9 fail. Constraint found: the process defines one unit of work — a slice of a `ready` contract inside a goal run — and sizes every ceremony for it, so other shapes either pay that toll (4 changelog lines → 26-check gate, #269) or have no written door and fall into the backlog (process/skill changes = 9 of the last 30 PRs; CI/toolchain changes). Unexplained residue recorded too: capture/drain asymmetry, and `concern` findings that every checkpoint emits and nothing consumes (dropped at `goal-run.js`'s verdict schema). Suite is a regression, not an opinion — re-run before changing a process rule; a lesson a real run pays for enters as a cell, once. Nothing filed to backlog by design (a per-finding item would build the second queue the shakedown looks for).
