...
- **AI mode MVP backlog — 1 epic + 3 items (all draft).** `docs/backlog/epics/ai-mode-mvp`: a diagnostic quality bench for rifty as an AI coding environment, not a leaderboard. `distribution/ai-mode-playground` (full in-playground AI mode: "+chat" and "vibe" views over one runtime, Pi agent loop + `pi-baseline+rifty-adapter-v1` prompts, standard coding-agent tools incl. both `edit_file` and `apply_patch`, no approve gate, any OpenAI-compatible endpoint + dev-only proxy, JSON session trace), `playground/react-vite-preset` (realistic issue-tracker SPA — React/TS/Router, portable unchanged to local Vite), `distribution/agent-bench-harness` (`tools/agent-bench`: Playwright runner, `prompt.md`/`seed/`/`judge.ts` task bundles, two lanes — built-in AI mode vs local Pi reference at same model+prompts, cold-start reset, 3 runs/task/lane → pass rate, `budget-exceeded` distinct from fail, per-run failure class `agent/rifty-runtime/rifty-tooling/ai-mode-ux/provider/task-bad`; judge hooks behind `?agentBench=1` under `__riftyAgentBench`, bench-only, not public API). Distilled from a 39-question grill session, 2026-07-02.
