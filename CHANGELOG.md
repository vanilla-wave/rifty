...
- e2e: `pickStarter`/`selectPreset`/`resetSandboxThroughUi` open the launcher through one `openLauncher` that waits for App readiness instead of a 2s deadline (hosted webpack flake on cold CI runners; closes `toolchain-build/hosted-webpack-launcher-readiness-timeout`).
...
- **Process: one evidence-driven route.** One verdict/validator at review and merge;
  ...
  at adoption, contract/result proof and disputed blockers. CLOSE resolves
  obligations, not journal lines. Supersedes the PR-4 referee split, ordinary
  verdict format, per-stage drivers and mandatory capture/report critics.

### Changed

- Process cut to the grammar it can keep consistent (2026-09-05, from the 2026-09-04 shakedown of `main` + PR #307: 13 rehearsal cells, 13 fail, ~36 silence steps — `docs/process/rehearsals.md`): every hole was the edge of a closed set (an enumerated ledger grammar, a number gate, an exclusion list, a goal-only container), and each patch added another set. Removed, not added: (1) the rounds budget and its ladder — `RDY-9`, `STOP-2`, `STOP-5`, `STOP-1c`/`STOP-1d`, `rounds:<n>`, the overwritten status lines, `goal-run.js` round arithmetic; with reception (`REV-12`) a pass with no FIX is already exit 0, so a counter could only count stalls, which `STOP-3` names — a stall re-cuts once (`STOP-4`), a stall surviving that makes the unit leave the path as a fog line `owner: user` (the map already holds questions; no `parked:` state, no in-memory set, CLOSE asks it at the latest); (2) the 15-row / 200-line size gate in `backlog:check` and the pickup split "before any review" — `REV-2` already bounds what a reviewer may block on, and the gate only manufactured checkpoint lineages (F ≈ 26 fresh contexts per successor, G cut the one chokepoint §Class-kill says to keep); one intent stays the rule, size a reviewer concern; (3) the tail pass keyed on `band ≥ 5` and the band row itself — the only numeric reader of a number with two definitions; `REV-10` axis 5 grades scope against the contract; (4) the goal directory as the only lifecycle container — a unit runs PICKUP → Contract+RED → IMPLEMENT → Final+GREEN on its own (a standalone item, a fix, four CHANGELOG lines are `review: ordinary` units with their own draft PR at the first commit, `PR-3`), a goal loops it over the map; its PASS lands in the container's journal (goal ledger rechart line, or the unit's `## Decisions` without a goal); (5) the ledger's enumerated line forms — one machine-read line (rechart PASS, `REV-8`), everything else `<date> — <gist>; <link>`; (6) the exclusion lists of `rifty-fix` / `rifty-refine` / `rifty-to-backlog` whose intersection was the void (docs-only edits, "is this mechanism still load-bearing?", an unreproducible red gate had no door); (7) `RDY-2`'s "one fresh context per contract, never two". Kept from #307: `REV-12` reception (FIX/REJECT/NOTE, a finding never becomes a unit on agent authority), "spend never asks", the compile-time trace test in `RDY-3`. Added, minimal: a `re-cut:` after a PASS ends its binding (`REV-8`); on a `proof-only` unit the reviewer grades a mutant-surviving row `missing` and its one review is fresh (`RDY-8`, `REV-4`, `REV-11`); the dedup scan reads `traps.md`; the proof-only product-diff check reuses `classifyAutonomousRunPath` instead of a hand-kept path list. Two fresh-context shakedowns of the cut tree (same suite; 22 then 27 defects, the second's new ones mostly consequences of the first round's patches) drove a second removal wave instead of more patches: `review:` has two values — `checkpoints` for a doc with traced product obligations, `ordinary` for a docs/CI/process/tooling subject and, by construction, for every unit with nothing to trace (a defect fix, a docs change, a CHANGELOG line, a CI rule: no doc is minted, nothing is journaled beyond the PR); `proof-only` is gone (a proof a named mutant survives is a `missing` carrier under `REV-4` as written, a product change inside a proof unit is `REV-1` scope); the ordinary review is one fresh prose review + inline reception + one verify pass, no schema, no `blockers.mjs`, no critic (`checkpoint-run.md` §Ordinary review — the machine verdict refused a unit with zero traced rows); `contract-drift` compares only the graded contract (status + User scenario … Out of scope) — the journal-line regex, the `fork:` check and the `blocked_by` closure special case are deleted, the fork discipline on user-traced rows is review's (`REV-10` axis 3), and a ready flip carries `ready-verdict:` or `review: ordinary`; `blockers.mjs` `REQUIRED_AXES[4]` follows the rubric (`Budget` → `Scope`; every checkpoint would have exited 2) and the test pins the names; a PASS binds through the CI docs-only classifier (`isDocumentationOnlyPath`) instead of a hand-kept exclusion list, so CLOSE exports and ADRs never break it; inside a goal a `STOP-1a` fork takes the same exit as a surviving stall (the unit leaves the path; a frontier empty behind it is where the question is asked; `STOP-1b`/`1e` halt), a unit without a goal stops on a fork and otherwise ends with a report; the `STOP-3` "2nd Contract+RED blocker = the contract is wrong" clause is gone; CLOSE may close short on a user-dropped invariant (`STOP-1e`); `## Parity cases` is required only where an oracle exists; a "keep it" answer to a question draft is a §Declined row at PICKUP; the fresh report subagent runs only for user-facing write-ups; `active-baseline change` (never defined) is gone. A third shakedown (5/13 cells pass whole, from 0/13) found 23 defects, 17 new; fixed: no standalone PASS line (the merged PR is the record), a `re-cut:` never voids the Contract+RED verdict (Final grades the contract as it stands), a unit leaving the path has its commits reverted on the goal branch and its fog line owned like its trace (`owner: user` for `I#`/scenario, `owner: agent` otherwise), the driver merges (`PR-3`, `DEC-3`), `blockers.mjs` drops coverage rows traced only to a rule id (`RDY-3`), the runner retries an invalid verdict once, `test:parity` reds are observed defects too. A child audit of `goal-run.js` against the prose listed 43 divergences (rule text restated in prompts, decisions taken in JS: review path, stall, re-cut count, retry, unblock policy) — the driver is rewritten as a pure sequencer: one exit schema for every stage (`done | pass | left-path | stop | harness`), prompts of the form "run <stage> per <doc>", 241 → 167 lines, nothing decided in JS; the stage docs name their exits, the runner reads stalls from the fix commits in git log (a re-invoked run continues), re-chart debt (no on-disk witness) is gone — RECHART follows the PASS in the same session. A fourth shakedown on that tree: 7/13 cells pass whole (E, F, G with its stall counterfactual newly), 20 defects, 13 new and one-line each — fixed: the driver reads a unit's stage by status first (journal lines are history), `pass` carries the sha for ordinary units too, a failed RECHART is a harness exit, an ordinary-unit stall follows `STOP-3`/`STOP-4` like any other, IMPLEMENT admits a no-doc unit and names the unlanded-red exit, the binding tolerates delete-on-done, a captured defect has one workflow (the fix PR deletes the draft), `blockers.mjs` accepts empty coverage. Left carried: adjudicated residuals never block; any historical `ready-verdict:` satisfies the flip gate; refine cannot decline a question itself. Suite banked in `docs/process/rehearsals.md`. From an outside read of the PR (2026-09-06): a stuck unit's fog line is owned by the `STOP-1` test, not by its trace — `owner: user` only when a named tradeoff or a fork exists, `owner: agent` when an approach failed (the source of a requirement never makes the user the owner of a technical dead end); a REJECT returns only with an executed artifact the ruling did not have (`REV-5`) and goes through reception again — the categorical "never re-raised" could freeze a critic's misread; a `pass`-graded row found broken later goes through reception, never ignored; a discovery outside the unit is repaired at once as its own unit or captured when it waits — capture is for deferring, not a toll; the rehearsal method counts silence as a fail only on machine paths or between conflicting principles — on an agent path a step derivable from one named principle passes, so the suite stops rewarding a rule per case. A fresh adversarial fidelity audit (2026-09-06: can an agent following the letter merge "something like the truth"?) answered no and ranked 12 holes; closed here: an `ordinary` unit's driver may FIX or NOTE on its own authority but a REJECT of a blocker citing a `REV-2` authority needs the fresh critic, and the reviewer's verdict is posted to the PR naming the sha (the ordinary route was self-adjudicated and artifact-free); `check:contract-drift` again refuses a ready flip beside production source without a Contract+RED verdict (`review: ordinary` never flips beside product — the widest door), refuses a dropped `→ I#`/`→ scenario` row without `fork:` (a count, so rewording stays review's), and treats `tools/node-parity-runner/src/**` and `.github/workflows/**` as referees (`PR-4`: the oracle and the lanes never ride as carriers); every checkpoint PASS commits its `verdict.json` + `adjudication.json` under `reference/`; a §Fidelity blocker is rejected only as FALSE with the carrier cited, never STRETCH, and a REJECT naming a product gap is a discovery; HOLDS covers a carrier an executed artifact shows non-discriminating; CLOSE's invariant proof is run by a fresh critic; a `## Parity cases` carrier is a runner case (or a real-Node artifact for browser-only behavior), a hand-typed golden a note; a pass-on-rerun is captured, never a silent flake; `REV-2` names §Fidelity whole and `RDY-8` misclassification. A second fidelity audit re-verified the twelve (1 closed, 10 narrowed, 1 outside the repo) and found six new, one grave and this PR's own: the rule-id filter in `blockers.mjs` matched `ADR-0375`, dropping every ADR-traced Fault-matrix row from the machine verdict — fixed (ADR traces are obligations), and with it: `blockers.mjs` refuses a STRETCH ruling on a §Fidelity blocker, keeps residuals blocking until every blocker is ruled (an empty adjudication neutralised them), and requires one coverage row per traced obligation of the contract it can read; `contract-drift` binds a `ready-verdict:` line to its committed `reference/<slug>-contract-red.json` (a self-written line is now a forged schema-valid artifact, not silence), checks `fork:` on the new re-cut line only (an old fork licensed every later drop), and treats the CI classifier, `ci-gate`, `pr-check`, `backlog:check`, the review rubric and the review skills as referees; a blocker is never NOTEd on an ordinary unit; referees never ride as carriers (`REV-1`); an ordinary verdict on a unit touching product or tests is committed under `reference/`; the driver reads `ready-verdict:` over `review: ordinary` when both are present; a stub or mutant experiment for a `REV-5` artifact runs in a detached worktree copy; a row-level obligation traced to `I#`/`scenario` is never dropped at CLOSE by the driver. A third audit found the structural flaw in that design: delete-on-done erased the doc and the artifact before the PR head merged, so with squash-merge no gate ever saw the flip, and the artifact check accepted a one-field JSON. Root fix: the verdicts stay on `main` (`reference/<slug>-{contract-red,final-green,ordinary}.json` — delete on done deletes the contract, never the record); a verdict is schema-shaped (eight axes, coverage, the unit it names) and carries `reviewed_sha`, which `check:contract-drift` binds to the `ready-verdict:` line in ANY diff (a two-PR split no longer dodges it) and which the new `check:pass-binding` (`tools/checks/pass-binding.mjs`, a CI step) requires at merge: a PR marked ready that changes product or test paths carries a landing verdict whose `reviewed_sha` is an ancestor of HEAD with only documentation changed since; draft PRs are skipped. A ready unit deleted beside production must leave that landing verdict. Referees are now the complement of what rides with product (product, tests, examples, deploy/perf, backlog/ADR docs, changelogs, lockfile, workspace/tsconfig) — an open rule, no growing list; `production` counts every shipped file under a product root incl. `tools/shadow-registry`. `blockers.mjs`: an empty adjudication never launders residuals, FALSE on a §Fidelity blocker cites the carrier as `file:line`, a named unreadable contract is invalid; the schema gains `reviewed_sha` and `findings[].evidence` (the executed artifact the critic reads). A dropped ADR-traced row names the ADR in its re-cut line; a hand-typed golden grades `missing`; the driver never sends an unverified unit past PICKUP. A fourth audit built the standard path in a scratch repo (flip + stub → delete-on-done → head) and watched both gates say OK: the aggregate diff read `draft → D`, the landing artifact was a 60-byte JSON, `ready_for_review` ran no CI and `pr:check` lacked the lane. Closed: a compiled draft deleted beside production must leave BOTH its Contract+RED and its landing verdict, a plain draft any landing verdict added in the diff; `check:pass-binding` validates the landing verdict's shape (eight axes / prose + reception with `by: critic|driver` for ordinary), refuses a re-pointed `reviewed_sha` on an unchanged file and a driver-REJECTed Fidelity blocker, and is a `pr:check` lane; CI runs on `ready_for_review`; parity cases (`tools/node-parity-runner/cases/**`) ride with the product as its RED and count as tests for the binding, the harness `src/` stays a referee; package-local `vitest`/`playwright` configs and `package.json`s are referees, and the referee check runs beside tests too; a `ready-verdict:` line added or changed on an already-ready unit binds like a flip; a dropped ADR row names THAT ADR; a Fidelity authority is written `AGENTS.md §Fidelity: …` or the verdict is invalid. Left for the user: branch protection requiring the CI gate at merge (ADR-0323) and reviewer identity behind a committed artifact — the two holes no rule can close; left in the canon: a count-preserving row swap (review's), the STOP-4 owner rule (a feedback decision), squash-merge orphaning `reviewed_sha` (the runner records `reviewed_tree` beside it). Canon size (README + rules + stages + artifacts + driver + skills): `main` 1557 lines, #307 1608, this tree ≈ 1690 — six mechanisms and three referee checks fewer; the lines went where the suite found silence (the standalone and no-doc lanes, the ordinary-review runner, the fork exit) and to `REV-12`.
- Codex gains the explicit-only `rifty-review-inline` skill, matching Claude's
  shared-context WIP review without checkpoint or merge authority.
- `pr:check` lanes follow the diff and time-outs rerun in isolation (2026-09-03): the gate classifies the working tree vs the `origin/main` merge-base with CI's classifier (`tools/checks/ci-change-scope.mjs`, which now also treats `.agents/**` and `.claude/**` as documentation, so docs-only skill/workflow PRs skip heavy CI too) — a docs-only diff skips `typecheck`, `build:libs`, `check:arch`, `test:run`, `test:parity` and names them; no `origin/main` → full gate (fail-open); `--all` forces the full gate. `test:run` runs with the JSON reporter; a red lane reruns its failed files once in isolation and reports `passed on isolated rerun` with the failure / vitest time-out counts and the host load average — a failure that reproduces in isolation stays red, an abort without a JSON summary never reruns. Evidence: docs-only PR #305 ran `test:run` three times, twice red on two unrelated time-outs (Vite production build 34.9 s > 30 s, parity-runner probe > 5 s) at load average 25–47 beside a neighbour worktree's gate, both green in isolation (5.8 s / 1.8 s) under the same host load — the contention is mostly the gate's own worker pool. `AGENTS.md` §DoD and `rules/pr.md` `PR-6` name the rule; `process-meta/pr-check-vitest-resource-starvation` narrows to the worker-count policy.
- Process cuts from the 2026-09-03 `no-coi-sandbox-tier` run (PR #304) and the 08-31…09-02 run audit (617 Codex sessions, 4.7B input / 94M uncached tokens): (1) `rifty-goal` no longer ends at a mode boundary — a whole-goal hand-off runs the §Stages loop in the hand-off session until the map is empty or a `STOP-1` item, and `stops.md` names the end of a stage as never a stop; the "one mode per invocation, fresh session per mode" clause put a Codex run to sleep for 2h21m after PICKUP + Contract+RED, the loop's only driver being Claude's `goal-run.js`. (2) `DEC-5` roles: fresh isolated context only where it is the evidence (reviewer, critic — `REV-11`); orchestrator + worker + runner are one driver session — "one role per session" was either that stop or silently violated (the same session did PICKUP, IMPLEMENT and the Final runner). (3) `DEC-5` waiting is harness-bound, overturning "sleeps ≥ 20 min, never polls per minute / background task + completion notification / work meanwhile": Codex clamps `exec_command` at 30 s and an empty `write_stdin` at 300 s (`MAX_YIELD_TIME_MS`, `DEFAULT_MAX_BACKGROUND_TERMINAL_TIMEOUT_MS`; probed 300.0 s) and kills unified-exec children at turn end, so the driver waits with one empty `write_stdin` per 5 min and nothing between polls — the audit counted 6 417 poll responses (21 % of responses, 38 % of input, ~10 % of uncached tokens) plus 26 status messages in 30 min, and an agent invented a "20-minute window" after which it would treat a live reviewer as an infra failure; a wait has no deadline. (4) `RDY-3`/`REV-2`/`REV-4`: a rule-id-only trace is a carrier note (no coverage row) and only rules that name the blocker themselves (§Fidelity, `REV-7`, `REV-11`, `REV-10` axis 3) are blocker authorities; everything else (`DEC-2` graft, `RDY-4`, `REV-8` forms) is a concern — the only blocker of the 09-03 slice was ADR prose (DEC-2 graft: one round, two reviewer runs, zero product lines); on #294, 15 HOLDS traced to no invariant. `backlog:check` keeps accepting the `→ <rule-id>` syntax; `blockers.mjs` is unchanged — the allowlist is applied by the reviewer and adjudication. (5) `DEC-2`/`AGENTS.md`/`docs/adr/README.md`: a new decision on an owned seam is a new short ADR citing the prior one — nothing removed, nothing grafted; supersede-by-removal only when every decision is overturned, partial overturn = dated §Corrections note — ADR-0373→0374→0375→0376 were four rewrites of one seam in three days, two deleted, the last graft a Final blocker. (6) `RDY-8` membership by delivery: a unit with no expected product delta (proof carriers, mechanism record, evidence, ADR) is `review: ordinary — proof-only` whatever its subject — one review, no Contract+RED, no rounds, its RED a mutant kill-list in `reference/`, the review verifying the product diff from `BASE` is empty; `STOP-4` split successors carrying only proof rows are minted that way; the landed record is `re-chart after <slice> (ordinary PASS @ <sha>)` (`REV-8`, `goal-run.js`) — the 09-03 proof-only slice took four reviewer runs and a round for zero product lines, and #294's admission / bounded-cause / descriptor children (8h27 / 4h03 / 3h31 for ≤ 19 product lines each) had the same shape. (7) `traps.md` `codex-sandbox-listen-eperm` + `implement.md`: `pr:check`, playwright and tsx lanes fail by construction inside the Codex workspace-write sandbox (`listen EPERM` on tsx IPC pipes and loopback) — run them escalated from the first attempt; every in-sandbox attempt cost 3–4 min (21 hits on #294, two on 09-03).
- Process re-cut into one map (2026-09-02, from the `no-coi-sandbox-tier` build-loop stops): the canon was ~1300 lines in 17 files across 5 places with no entry point, the convergence valve defined in 6 files with two formulas (prose: 2 rounds; `goal-run.js`: 3), and a stage's procedure split across a skill file, `decision-workflow.md`, the backlog README and a workflow prompt. Observed cost on one goal: a `ready` unit became a second frozen layer (any weakening = user fork), so the agent paged the user three times in a day to authorize splits it owned; the unit contract grew to 953 lines and a reviewer obliged to grade every clause produced 15 "prove more exactly" blockers none of which traced to an invariant; the count-delta valve fired on `1→1` twice and on `1→15` after a boundary change; each authorized split cost a full checkpoint pair (~3h). New layout `docs/process/`: `README.md` (layers / roles / stages / closed stop list / rule index), `stages/` (fit, pickup, contract-red, implement, final-green, checkpoint-run, rechart, close — input → output → gate), `artifacts/` (goal, map, ledger, unit, verdict — shape + owner/editors per section + line forms), `rules/` (`DEC` decisions, `RDY` readiness, `REV` review, `STOP` stops, `PR`, fault-classes, testing — one home per rule, cited by id). Skills are thin routers (`rifty-goal` mode files deleted); `AGENTS.md` points at the map; `decision-workflow.md` and `fault-classes.md` stay as stubs for immutable ADR citations. Rule changes: (1) `RDY-3` trace — every Acceptance/Parity/Fault row names why it exists (`→ I#` / `→ scenario` / `→ ADR` / `→ rule id`); untraced rows are notes, never coverage rows or blockers; (2) `RDY-4` size — one intent per unit, ≤ 15 traced rows, ≤ 200 lines, run state (evidence blocks, fork narratives) lives in ledger/`reference/`; (3) `RDY-5` re-cut is agent-owned at any time with a `re-cut:` line — only dropping/weakening a row traced to `I#`/`scenario` is the user's (fork → `rifty-refine`); `check:contract-drift` now verifies the re-cut line and a `fork:` marker on user-traced changes instead of freezing ready-contract text; (4) `REV-3` severity — `concern` is advisory and never spends a round, `weak` coverage is report-only, `missing` blocks; coverage rows carry `trace` (`blockers.mjs`, `review-schema.json`); (5) `STOP` — the count-delta convergence valve and "valves take no budget argument by design" (2026-08-31) are overturned: rounds are declared per unit at pickup (`RDY-9`, defaults 1/2), a stalled blocker skips the remaining rounds, both route to ONE agent-owned re-cut against the destination (`STOP-4`), and only blockers surviving that re-cut stop to the user; `STOP-1` is the closed list of user stops (observable-scope fork, premise, post-re-cut exhaustion / 2nd escalation, slice cap, destination conflict) — push, draft PR, split, re-cut, band/rounds, rechart never ask; (6) `DEC-5` one role per session, decisions written to the journal before they can leave context, no per-minute polling of long children; (7) split successors carrying only certified rows inherit Contract+RED, band ≤ 4 gets a single find pass. Machine gates: `backlog:check` enforces trace + size on ready items `created ≥ 2026-09-03`; `contract-drift` treats lineage lines (`ready-verdict`/`contract-red`/`final-green`/`review`/`re-cut`/`override`) as journal. (8) `REV-8` lineage cut to three records: `ready-verdict:` at pickup, one overwritten status line per checkpoint (`final-green: round n/budget — blocker @ sha`, committed with the fix batch — no verdict commits), and the Final PASS inside the mandatory `re-chart after <slice> (final-green PASS @ <sha>)` ledger line, which is also the next slice's `BASE`; binding = `git diff --quiet <sha> HEAD -- . ':!docs/backlog' ':!CHANGELOG.md'` instead of hand-written tree hashes; split successors name their predecessor, copy nothing. On #294 that was 16 verdict-bookkeeping commits of 83 and ~60 commit/tree hashes in prose. Field references: Spec Kit (constitution vs regenerated spec/plan/tasks; only CRITICAL blocks), Wayfinder (destination frozen, route agent-owned, findings quote the spec, never loop review to clean), Shape Up (appetite + circuit breaker + scope hammering), BMAD (severity-graded advisory gates, `decision_needed` only for intent ambiguity).
- Probe-or-fog was a false dichotomy (2026-08-31, from the `no-coi-sandbox-tier` run stop): FIT offered every assumption exactly two exits — probe or fog — and neither is "ask the user", though FIT is interactive and the user is present. The 2026-08-28 FIT critic named the cheaper rival route (a coi-serviceworker shim delivering full COI) as problem 1; four of its five problems were routed to the user, P1 was routed to a probe because one existed for its technical half — while the deciding half ("does the value require the host page to stay non-COI?") is pure observable scope and was answerable in one sentence. Its fog line asked both halves in one sentence and exited on the technical one. Three days later the probe collapsed mid-run, the run stopped, and the goal document contained no clause excluding the rival route, so the agent concluded the epic was near-pointless and the user had to hand-feed the discriminator. Fix: owner-typing precedes the probe-or-fog choice — user-owned observable scope answerable now is ASKED at FIT (a mixed question is split; an answer that could invalidate the destination is never fog, since a frozen goal's only in-run exit is close + re-fit); fog lines carry `owner: user|agent` and `owner: user` routes to `rifty-refine` at PICKUP, never to a probe; the answer lands checkable as an invariant or `rejected route: <route> — violates <I#>`; a premise objection at a checkpoint is a concern that stops the run to the user, never a blocker/round/attempt, answered by citing that line. `rifty-goal` FIT 3-4 + PICKUP 2, `docs/backlog/README.md` §Epic fit, `epics/TEMPLATE.md`, `fault-classes.md` §Review convergence (Premise row), `rifty-review` axis 2. No new gate; `backlog:check` unchanged.
- draft→ready timing made explicit (2026-08-31): the docs stated the transition but never WHEN it fires, so a reader (agent or human) reads the pipeline as intake → ready items → hand-off. Actual boundary: the ready `goal.md` — a goal hands off with its children still `draft`, each compiling at its own PICKUP, JIT per slice; FIT never compiles a child; outside a run a settled draft may compile at intake, verdict still the unit's Contract+RED at pickup. `decision-workflow.md` §Backlog readiness (new WHEN paragraph), `docs/backlog/README.md` §Epic fit, `rifty-goal` FIT 5. No gate or behavior change.
- Contract+RED stall fix (2026-08-31), from two non-converging runs (#289 recursive contract split; `no-coi-sandbox-tier` 12 rounds, 115 blockers, 0 product lines): the pre-implementation checkpoint asked the reviewer to imagine a wrong implementation of code that does not exist yet — an infinite generator — while the review boundary was borrowed from the delivery form. Four changes, no budgets or counters: (1) review scope is the unit of work, not the PR — Contract+RED reviews the contract + its RED, Final+GREEN the slice diff from the prior landed slice's tree; certified slices and out-of-scope carriers (docs/CI/tooling/harness) raise no coverage row and never block; (2) the adversarial mutant judgement stays at Final+GREEN, where code settles it — Contract+RED admits four blocker classes, each carrying an executed artifact (false oracle fact / uncovered User-scenario line / RED not failing or failing wrongly / RED passing unimplemented), matching the bar the contract itself carries; (3) two valves end the round loop instead of grinding — Contract escalation (2nd consecutive Contract+RED blocker; 2nd escalation in a lineage stops to the user) and Convergence (blocker count not strictly falling across two rounds = fixes grow the surface, restored from the 2026-07-11 guard dropped on 2026-07-25), with attempt counts recorded in the unit doc so the count is observable and `goal-run.js` taking no round-budget arg; (4) two `## Challenge` premise classes (value does not follow / materially cheaper direct authority) now block `draft→ready`, closing by answer or recorded user override. Plus (5) checkpoint membership is decided once per unit at pickup and recorded (`review: checkpoints|ordinary`) — a run loop that checkpointed every child regardless is what put a CI/harness unit (`no-coi-substrate-lane`) through 12 Contract+RED rounds it never owed; in/out is judged by ROLE in the unit, so the unit's own RED counts even under `tools/`. Replayed against both stalled trees with these rules: `no-coi-substrate-lane` went 11 blockers → 0 (32/32 coverage pass, 4 ordinary-review concerns); `esbuild-pack-member-differential-proof` returned 6 blockers, each carrying an executed mutant run instead of an imagined encoding, plus a concern that several sibling units were flipped `ready` against their own Challenge. `fault-classes.md` §Review convergence, `rifty-review` §Checkpoint run, `docs/backlog/README.md` §Challenge, `decision-workflow.md` §Backlog readiness + §Goal runs, `rifty-goal` PICKUP, `.claude/workflows/goal-run.js`.
- Retro #256 (2026-08-29): probe-or-fog widened to internal scope-carrying mappings ("path X is the issue scenario"), not just external semantics — the #256 epic-critical miss rode such a mapping frozen unverified (`rifty-goal` FIT §3, `decision-workflow.md` §Goal runs); reviewer + test batteries run as background tasks with completion notification, never fixed-interval sleep/poll loops — idle waiting is a defect, 10.8h of 22h slept in polls (`rifty-review` §Checkpoint run). Blocker-bar and prior-verdict-carry parts of the retro superseded by the landed authority-certification scheme (#274/#275 retro entry below).
- Backlog input re-cut to a single human window (2026-08-29): `rifty-refine` = the entry for user-brought ideas/findings — dedup-scan → grill (frontier rounds; no fork → skip, formalize-only) → `rifty-to-backlog` formalize tail, one invocation; `rifty-to-backlog` direct invocation narrows to mid-task/agent intake (never interviews — fork recorded + manual refine requested); refine flips to implicit-invocable (openai policy). Routing: `AGENTS.md` §Data sources. Machine paths unchanged: `decision-workflow.md` §Backlog readiness, goal-run workflow, `rifty-goal` PICKUP/RECHART/FIT, `rifty-fix`.

- Goal signoff gate removed (2026-08-28): a ready `goal.md` needs Outcome/User scenario/Invariants/tier only — no user `signoff:` line; FIT flips `status: ready` itself and every backlog write-up (capture, FIT, re-fit) ends with a mandatory user-facing completion report (what was recorded, challenge problems verbatim, what happens next) — a report, not an approval ask; pushback re-opens FIT. `docs/backlog/README.md` §Report + §Epic fit (canon), §Challenge, `rifty-goal` FIT step 8, `rifty-to-backlog` step 6, `goal-run` workflow gate, `decision-workflow.md` §Goal runs. Machine gates unchanged (`backlog:check` never enforced signoff).

- Goal-run scheme re-cut (2026-08-28): PR count per goal is a weak rule — one draft PR per goal by default, opened at the goal's first Contract+RED and carrying all slices; splitting allowed, never required (`AGENTS.md` §PR, `docs/backlog/README.md` §Goal run; "landed" = Final+GREEN on the goal branch — `rifty-goal`/`rifty-review` merged→landed). Claude sessions may drive the loop deterministically via the `goal-run` workflow (`.claude/workflows/goal-run.js`): slice loop until the map is empty, then CLOSE; any user-owned decision (fork/refine, signoff, caps) returns a structured stop, re-invoke resumes off disk state; skills stay canon — the script owns only order, gates, bookkeeping. FIT stays inline (interactive). While a goal is in a run the invoking session is an observer: launch/re-invoke + brief status relay only, no tracked-file edits (a hand edit voids tree-bound checkpoint verdicts); sole exception — named stop-resolution with the user (`rifty-refine`).

- Backlog intake gains an advisory premise-challenge gate (retro of #279: hop-count hypothesis was never sized against the whole and no actor had a mandate to attack it — every gate certifies conformance to a frozen goal, none its correctness): every new item/epic (`created` ≥ 2026-08-27) carries `## Challenge` — one fresh independent critic attacks the premise (value follows? impact sized against the whole? cheaper route?) and names problems explicitly, especially user experience and project direction; verdict advisory, surfaces verbatim at FIT signoff and in the capturing PR body, never blocks. `docs/backlog/README.md` §Challenge (canon), `rifty-to-backlog` step 5, `rifty-goal` FIT step 7 + RECHART graduate, templates; `backlog:check` enforces presence + verdict line (created-date cutoff, older docs grandfathered); three 2026-08-27 perf drafts backfilled with live critic verdicts.

- `rifty-goal` CLOSE: ledger walk extends to `map.md` `## Open questions` fog lines (carrier or explicit drop each; fired trigger → carrier, never drop); recorded before/after numbers require a direction verdict, not a bare table (retro of #279 goal close: three fog lines died with `map.md`, after-numbers regressed with no statement). Same clause mirrored in `docs/backlog/README.md` §Goal run.
...
- **Shared-memory-free no-COI sandbox tier.** Existing headerless Chromium apps can create a capability-reported SDK sandbox, install and build real Vite 7, mount HMR preview, persist acknowledged OPFS writes across reload and explicitly restart a wedged realm without changing host COOP/COEP posture. Proof: `docs/backlog/distribution/reference/no-coi-sandbox-tier-closure-evidence.md`.
