...
export const judge: TaskJudge = async (ctx) => {
  await issues(ctx);
  const button = ctx.view.getByRole('button', { name: /new issue/i });
  ...
  if (!(await title.count()))
    return verdict([{ name: 'title field exists', pass: false, evidence: { title: false } }]);
  ...
  const nativeRequired = await title.evaluate(
    (node) =>
      node instanceof HTMLInputElement && !node.validity.valid && node.validationMessage.length > 0,
  );
