 ...
 *
 * The Node runner executes this case under an OS pseudo-terminal and maps the
 * hook to `stty`; the rifty runner maps it to the kernel IPC resize frame. The
 * case code is identical on both sides, so dimensions, getWindowSize(), and
 * observable event order stay pinned to real Node.
 */
const c: ParityCase = {
  kind: 'tty-resize',
  expected:
