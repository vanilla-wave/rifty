...
function tsupConfig(entries, spec) {
  ...
  const entryBody = (selectedEntries, indent = '    ') =>
    Object.entries(selectedEntries)
      ...
      .join('\n');
  const browserWorkerEntries = spec.browserWorkerEntries;
  if (browserWorkerEntries !== undefined) {
    const unknownWorkerEntries = browserWorkerEntries.filter(
      (entry) => !Object.hasOwn(entries, entry),
    );
    if (unknownWorkerEntries.length > 0) {
      throw new Error(`Unknown browser worker entries: ${unknownWorkerEntries.join(', ')}`);
    }
    const workerEntries = Object.fromEntries(
      browserWorkerEntries.map((entry) => [entry, entries[entry]]),
    );
    const apiEntries = Object.fromEntries(
      Object.entries(entries).filter(([entry]) => !browserWorkerEntries.includes(entry)),
    );
...
for (const [name, spec] of Object.entries(SPEC)) {
  ...
  const nextTsup = tsupConfig(entries, spec);
  if (checkOnly) {
