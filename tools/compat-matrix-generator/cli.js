#!/usr/bin/env node
// Static inventories; TODO(backlog: toolchain-build/compat-matrix-test-result-sink).
...
function policyArray(value, label) {
  if (!Array.isArray(value)) throw new Error(`esbuild runtime policy: ${label} must be an array`);
  return value;
}
...
const matrices = [
  ...
  },
  {
    ...
    rows: [
      ...
      [
        '`http.Agent` socket pool',
        '❌',
        "Default and named HTTP facades share a constructor; import and subclass declarations are safe. Construction, including subclass super(), throws `NotImplementedError('node:http.Agent')`; no fake pooling (ADR-0459).",
      ],
      [
