...
1. **`AGENTS.md`** (this file) — vision + rules for the session
...
### Subagent delegation budget

- Subagents are allowed for independent research, review, verification, and scoped implementation work.
- Default max depth: 1. Depth 2+ needs explicit current-task permission.
- Depth 3 is read-only only (research / audit / verification / map-reduce). Depth >3 is forbidden unless the user explicitly overrides.
- Every subagent prompt must include: current depth, max depth, child-agents allowed yes/no, task mode (read-only/code-edit), and owned scope.
- Leaf agents must not spawn child agents; if they need more delegation, report that upward.
- Code-editing agents need disjoint file/module ownership. They must not revert or overwrite other agents' work. Parent agent owns integration, architecture calls, and final verification.
