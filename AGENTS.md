...
## Data sources
- `docs/backlog/` — provisional contracts: items + user-value epics; delete on done. Route: user-brought idea/finding (in session) → `rifty-refine` — single human entry: dedup-scan → grill observable forks (none → skip) → `rifty-to-backlog` formalizes; mid-task/agent discovery → `rifty-to-backlog` directly, never interview — unresolved observable fork → request manual `rifty-refine`; settled draft → compile to `ready`, verified at pickup Contract+RED (`decision-workflow.md` §Backlog readiness); unsettled fork inside a `ready` item → demote first (same §); epic missing tier/Invariants → fit it yourself (`rifty-goal` FIT) — a write-up is never a blocked ask; ready → implement normally; PR review → `rifty-review`. Never implement a draft. Planned/process work never invokes `rifty-fix`.
...
- Goal run: the goal is the delivered behavior — slices ride the goal's single draft PR by default; one or several PRs, splitting allowed, never required (`docs/backlog/README.md` §Goal run).
