...
- [ ] `npm install vite && npm run dev` literally running upstream Vite — Vite has hundreds of transitive deps and many edge cases; the equivalent dev-server is the architectural acceptance. Real Vite likely lands incrementally with `unenv` polyfills + esbuild.wasm (restored from swc per ADR-0047).
...
- **Go-runtime (gojs) bridge — deferred per ADR-0044 D3 (still valid; now moot for esbuild per ADR-0047 D3).** The `esbuild-wasm` package (0.21.5 / 0.25.0 / 0.28.0) targets Go's `js/wasm` ABI (`gojs.runtime.*` / `gojs.syscall/js.*`), but **esbuild no longer needs it** — we run the separate `@esbuild/wasi-preview1` WASIp1 build on the existing shim (ADR-0047). The Go-runtime bridge (`@rifty/runtime-go-wasm`: full `syscall/js` handle protocol, `wasm_exec.js`-equivalent host shim, GC + goroutine scheduling) is only relevant if some *other* gojs guest appears. Multi-week design; blocks nothing. Pick up when a Go-WASM guest with no WASI build actually shows up.
