...
- Approach earns its cost — even an on-goal capability is wrong if its implementation is disproportionate or net-negative for user or project; flag and reconsider, don't force it. Opposite probe: name what the diff could deliver the same contract without (a mechanism, layer, guard, config knob) — with no loss to goal, project evolvability, or user experience. Superfluous coordination/mechanism → blocker (`fault-classes.md` §Class-kill); mere shrinkage → concern + backlog item, never a new review round or merge condition
