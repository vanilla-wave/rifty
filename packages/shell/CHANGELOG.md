...
- **One live command resolver across execution and discovery (ADR-0362).**
  ...
  `commandNames`, typo suggestions, and the new `Shell.complete`, including
  active argv-0 after operators/env prefixes. Direct misses, directories, and
  non-directory path components retain distinct 127/126 diagnostics; resolved
  entries reach `BinExecutor` as normalized absolute paths, while `help` remains
  a registered-command synopsis.
...
- **`help` builtin.** `help` lists the live registered command registry (sorted;
  includes host-registered `node`/`npm`/`vite`) plus a note that those run
  programs; `help <cmd>` prints a one-line synopsis, unknown topic →
  `help: no help topic for '<cmd>'` exit 1. It intentionally excludes installed
  `.bin` discovery names. Guards: `help.test.ts`.
### Changed
