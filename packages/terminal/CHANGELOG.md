...
- Alt-tier kill-ring keys (ADR-0101): `macOptionIsMeta` is now exposed through
  ...
  kill ring.
- Bracketed paste policy (ADR-0102): line-mode terminal sets xterm's
  `ignoreBracketedPasteMode` and strips pasted `\x1b[200~...\x1b[201~` wrappers
  before they can enter the command buffer.
- Line-edit undo (ADR-0103): Ctrl+_ and delivered Ctrl+Z restore the previous
...
- xterm addon drop-ins (ADR-0105): `RiftyTerminal` now loads official xterm
  ...
  internals.
- Host line injection (ADR-0106): `replaceLine()` redraws the current prompt
...
- OSC 8 link handling (ADR-0111): xterm OSC 8 hyperlinks now route through the
...
- Command-line highlighting seam (ADR-0112): hosts may provide
...
- Multiline input validator (ADR-0113): hosts may provide
...
- Abbreviations/snippets (ADR-0118): hosts may provide
