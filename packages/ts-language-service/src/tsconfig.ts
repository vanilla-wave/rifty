...
function makeParseConfigHost(fsSync: FsSync): ts.ParseConfigHost {
  return {
    useCaseSensitiveFileNames: true,
    fileExists: (p) => fileExists(fsSync, p),
    readFile: (p) => readFileUtf8(fsSync, p),
    readDirectory: (rootDir, extensions, excludes, includes, depth) =>
      vfsReadDirectory(fsSync, rootDir, extensions, excludes, includes, depth),
  };
}
 ...
 */
export function loadTsConfig(fsSync: FsSync, projectRoot: string): ts.ParsedCommandLine {
  const host = makeParseConfigHost(fsSync);
  const configPath = ts.findConfigFile(projectRoot, (p) => fileExists(fsSync, p), 'tsconfig.json');
  ...
  if (!configPath) {
    ...
    return ts.parseJsonConfigFileContent({}, host, projectRoot);
  }
