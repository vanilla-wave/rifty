...
// isIP/isIPv4/isIPv6: regexes ported verbatim from Node lib/internal/net.js
// (incl. `%zone` suffix and RegExp ToString coercion of non-string input) —
// parity pinned by cases/net/is-ip.case.ts. On the hot path of every guest
// dev-server request: vite's host check calls net.isIP before its
// unconditional localhost allow; a throw there is swallowed by connect's
// async middleware chain and the request hangs with no response.
...
export function isIPv4(input: string): boolean {
  return IPv4Reg.exec(input) !== null;
}

export function isIPv6(input: string): boolean {
  return IPv6Reg.exec(input) !== null;
}
