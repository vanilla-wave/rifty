...
describe('package-private npm dev-server project definition Contract+RED reach', () => {
  it('mints a preview-ready zero-field definition without widening the public root', () => {
    expect(typeof defineNpmDevServerProject, 'npm dev-server definition capability').toBe(
      'function',
    );
    if (typeof defineNpmDevServerProject !== 'function') return;
...
describe.skipIf(typeof defineNpmDevServerProject !== 'function')(
  'package-private npm dev-server definition regression and faults',
  () => {
    it.each([
      ['missing manifest', {}],
