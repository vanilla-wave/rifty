...
describe('npm dev-server Playground definition Contract+RED', () => {
  it('owns exactly the common public plan fields with one zero-field discriminant', () => {
    expect(
      () => ownPlaygroundProjectPlan(plan(), URL_CONTEXT),
      'valid npm dev-server plan must be admitted',
    ).not.toThrow();
...
const supportsNpmDevServerPlan = (() => {
  try {
    ownPlaygroundProjectPlan(plan(), URL_CONTEXT);
    return true;
  } catch {
...
describe.skipIf(!supportsNpmDevServerPlan)(
  'npm dev-server Playground definition regression and faults',
  () => {
    it.each([
