...
function validateWorkbenchOptions(
  ...
  const wasm = exactPlainObject(deployment.wasm, 'deployment.wasm');
  const wasmKeys = Reflect.ownKeys(wasm);
  if (wasmKeys.length !== 1 || wasmKeys[0] !== 'sqlite') {
    throw new TypeError('deployment.wasm must contain exactly sqlite');
  }
  const sqliteWasm = enumerableDataProperty(wasm, 'sqlite', 'deployment.wasm.sqlite');
  ...
  return Object.freeze({
    ...
    owner: Object.freeze({
      deployment: Object.freeze({
        ...
        wasm: Object.freeze({
          sqlite: wasmAssetUrl(sqliteWasm, 'deployment.wasm.sqlite', urlContext),
        }),
