...
describe('openWorkbench normalized composition', () => {
  it('rejects the retired deployment.wasm.esbuild field before host effects', async () => {
    const h = harness();
    const options = validOptions();
    const withRetiredEsbuild = {
      ...options,
      deployment: {
        ...options.deployment,
        wasm: { ...options.deployment.wasm, esbuild: '/assets/esbuild.wasm' },
      },
    } as unknown as WorkbenchOptions;

    await expect(h.open(withRetiredEsbuild)).rejects.toThrow(/deployment\.wasm.*exactly sqlite/i);
