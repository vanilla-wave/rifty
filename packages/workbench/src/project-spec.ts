...
export function terminalDevLine(spec: ProjectSpec, root: string): string {
  ...
  return `cd ${shellWord(root)} && npm run dev`;
}

/** Real visible boot sequence for one configured workbench project. */
export function terminalBootLines(
  spec: ProjectSpec,
  root: string,
  setup: 'instant' | 'from-scratch',
): readonly string[] {
  const dev = terminalDevLine(spec, root);
  if (setup === 'instant') return [dev];
  const bareDev = spec.runtime === 'vite' ? dev : 'npm run dev';
  return [`cd ${shellWord(root)} && npm install && ${bareDev}`];
}
