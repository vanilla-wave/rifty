...
import { type Fetcher, RegistryClient, install } from '@riftydev/npm-client';
...
import { esbuildShimFiles, rollupShimFiles } from '@riftydev/shadow-registry';
...
import { serveNodeModulesReads } from './node-modules-port.ts';
...
import { DEFAULT_TEMPLATE_ID, resolveProjectSpec } from './registry.ts';
import { SyncMirrorVfs } from './sync-mirror-vfs.ts';
import { collectSnapshot, publishVfsSnapshot } from './vfs-snapshot-port.ts';
import { type VfsWriteFrame, applyVfsWriteFrame, serveVfsWrites } from './vfs-write-port.ts';
import { type ViteModuleGraph, invalidateViteModule } from './vite-invalidation.ts';

const enc = new TextEncoder();

export interface ProjectWorkerShimFiles {
  readonly esbuild?: Record<string, string>;
  readonly rollup?: Record<string, string>;
}

export interface ProjectWorkerOptions {
  readonly registryFetch?: Fetcher;
  readonly sqlWasmUrl?: string;
  readonly shimFiles?: ProjectWorkerShimFiles;
...
function overlayShims(shimFiles: ProjectWorkerShimFiles): void {
  const fs = syncMirror();
  for (const [path, content] of [
    ...Object.entries(shimFiles.esbuild ?? esbuildShimFiles),
    ...Object.entries(shimFiles.rollup ?? rollupShimFiles),
  ]) {
    const np = normalizePath(path);
    fs.mkdirSync(dirname(np), { recursive: true });
    fs.writeFileSync(np, enc.encode(content));
...
async function bootNodeServer(
  ...
  loader: Loader,
  opts: ProjectWorkerOptions,
): Promise<void> {
  if (cfg.sqlite) {
    if (!opts.sqlWasmUrl) {
      throw new Error('[real-vite/worker] sql.js wasm url is required for sqlite templates');
    }
    ...
    const wasmResponse = await fetch(opts.sqlWasmUrl);
    if (!wasmResponse.ok) {
...
export async function runProjectWorker(opts: ProjectWorkerOptions = {}): Promise<void> {
  registerNetBuiltins();
  ...
  await ensureProjectDependencies({
    ...
    install: async () => {
      ...
      const registry = new RegistryClient({
        fetch: opts.registryFetch ?? createRegistryProxyFetch({ proxyPrefix: '/npm-registry' }),
      });
  ...
  if (cfg.runtime === 'node-server') {
    await bootNodeServer(cfg, loader, opts);
    publishSnapshot();
  }

  if (cfg.runtime === 'vite') {
    overlayShims(opts.shimFiles ?? {});
  ...
  if (opts.keepAlive ?? true) await new Promise<never>(() => {});
}
