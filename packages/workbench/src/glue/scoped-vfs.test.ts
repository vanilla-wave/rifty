...
describe('workspace-scoped VFS', () => {
  it('maps public absolute paths under a workspace prefix', () => {
    ...
    expect(fsSync.existsSync('/workspaces/ws%2Fa%3Ab/scratch/main.js')).toBe(true);
    expect(dec.decode(fsSync.readFileBytesSync('/workspaces/ws%2Fa%3Ab/scratch/main.js'))).toBe(
      'scoped',
    );
  });

  it('encodes workspace ids injectively without path-segment traversal', () => {
    expect(workspaceVfsPrefix('.')).toBe('/workspaces/%2E');
    expect(workspaceVfsPrefix('..')).toBe('/workspaces/%2E%2E');
    expect(workspaceVfsPrefix('a/b')).toBe('/workspaces/a%2Fb');
    expect(workspaceVfsPrefix('a_b')).toBe('/workspaces/a_b');
    expect(workspaceVfsPrefix('a%2Fb')).toBe('/workspaces/a%252Fb');
    expect(new Set(['a/b', 'a_b', 'a%2Fb'].map(workspaceVfsPrefix))).toHaveLength(3);
    expect(() => workspaceVfsPrefix('\ud800')).toThrow(/workspaceId/);
  });
