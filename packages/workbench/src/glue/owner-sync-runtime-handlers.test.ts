...
describe('project-rooted recursive execSync namespace', () => {
  it('preflights the logical guest path while retaining the physical nested-fs relay', async () => {
    const { fsSync } = createMemoryFs();
    fsSync.mkdirSync(ROOT, { recursive: true });
    fsSync.writeFileSync(`${ROOT}/child.mjs`, enc.encode('process.stdout.write("child")'));
    const guest = new ProjectTerminalFsSync(fsSync, ROOT);
    const dispatcher = new SyncRpcDispatcher({ pollIntervalMs: 60_000 });
    let runSpec: {
    ...
    installOwnerSyncRuntimeHandlers(dispatcher, () => fsSync, {
      execSyncVfs: () => guest,
      execSync: {
        remoteFsRoot: () => ROOT,
        runWorker: async (spec) => {
          ...
          return { stdout: enc.encode('child'), exitCode: 0 };
        },
      },
    });
    ...
    ).resolves.toEqual({ ok: true, value: enc.encode('child') });
    expect(runSpec).toEqual({ entryPath: '/child.mjs', cwd: '/', remoteFsRoot: ROOT });
  });
});
