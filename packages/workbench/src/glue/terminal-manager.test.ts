...
import type { ExecOptions, PtySessionSnapshot } from './pty-client.ts';
...
function makeFakeOwner(opts: { readonly root?: string } = {}) {
  ...
  const resized: Array<{ sid: string; rid: string; cols: number; rows: number }> = [];
  ...
  const owner: WorkspaceOwnerHandle = {
    ...
    resize(sid: string, rid: string, cols: number, rows: number): void {
      resized.push({ sid, rid, cols, rows });
    },
  ...
  return { owner, opened, closed, stdin, signalled, resized, writes, execs, snapshots };
}
...
describe('createTerminalManager (pty port client)', () => {
  ...
  it('reports a throwing terminal writer without corrupting the foreground run', async () => {
    const reportError = vi.fn();
    vi.stubGlobal('reportError', reportError);
    const fake = makeFakeOwner();
    const manager = createTerminalManager({ owner: fake.owner });
    const session = manager.sessions()[0]!;
    const writerError = new Error('terminal renderer failed');
    manager.attachWriter(session.id, () => {
      throw writerError;
    });

    const run = manager.runLine(session.id, 'echo hi');
    await waitForExecs(fake.execs, 1);
    expect(() => fake.execs[0]!.opts.onChunk('hi\n', 'stdout')).not.toThrow();
    fake.execs[0]!.resolve(0);

    await expect(run).resolves.toBe(0);
    expect(reportError).toHaveBeenCalledWith(writerError);
    expect(manager.snapshot(session.id)).toMatchObject({ status: 'idle', exitCode: 0 });
    manager.dispose();
    vi.unstubAllGlobals();
  });
  ...
  it('uses the latest dimensions for exec and forwards live resize to the active run', async () => {
    const fake = makeFakeOwner();
    const manager = createTerminalManager({ owner: fake.owner });
    const session = manager.sessions()[0]!;

    manager.resize(session.id, { cols: 100, rows: 30 });
    const run = manager.runLine(session.id, 'watch-size');
    await waitForExecs(fake.execs, 1);
    expect(fake.execs[0]!.opts).toMatchObject({ cols: 100, rows: 30 });

    manager.resize(session.id, { cols: 132, rows: 43 });
    ...
    manager.resize(session.id, { cols: 140, rows: 50 });
    expect(fake.resized).toHaveLength(1);
    const next = manager.runLine(session.id, 'next');
    await waitForExecs(fake.execs, 2);
    expect(fake.execs[1]!.opts).toMatchObject({ cols: 140, rows: 50 });
    fake.execs[1]!.resolve(0);
    await next;
  ...
  it('closes every pty session on dispose and throws for post-dispose methods', async () => {
    ...
    expect(() => manager.resize(session.id, { cols: 100, rows: 30 })).toThrow(
      'Terminal manager is disposed',
    );
  ...
  it('attempts every PTY signal and close before aggregating dispose failures', async () => {
    const fake = makeFakeOwner();
    const manager = createTerminalManager({ owner: fake.owner });
    const first = manager.sessions()[0]!;
    const second = manager.createSession('Second');
    const firstRun = manager.runLine(first.id, 'first');
    const secondRun = manager.runLine(second.id, 'second');
    await waitForExecs(fake.execs, 2);
    const signalAttempts: string[] = [];
    const closeAttempts: string[] = [];
    fake.owner.signal = (sid) => {
      signalAttempts.push(sid);
      if (sid === first.id) throw new Error('first signal failed');
    };
    fake.owner.closeSession = (sid) => {
      closeAttempts.push(sid);
      if (sid === first.id) throw new Error('first close failed');
    };

    expect(() => manager.dispose()).toThrow(AggregateError);
    expect(signalAttempts).toEqual([first.id, second.id]);
    expect(closeAttempts).toEqual([first.id, second.id]);
    expect(() => manager.dispose()).not.toThrow();

    for (const exec of fake.execs) exec.resolve(130);
