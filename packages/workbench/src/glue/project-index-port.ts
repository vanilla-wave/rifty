...
export function serveProjectIndex(
  ...
  const catalogForSeed = (): WorkbenchProjectCatalog => {
    if (!catalog) throw new Error('serveProjectIndex: project catalog is required for reset/seed');
    return catalog;
  };
  ...
  const resetScratch = (starter: string, opId?: string): void => {
    ...
    if (index.scratch) {
      ...
      const source = catalogForSeed();
      resetScratchToStarter(fs, seedFilesForStarter(source, resolveStarter(source, starter), root));
      writeIndex(fs, base, {
        ...index,
  ...
  const resetProjectTree = (projectId: string, opId?: string): void => {
    ...
    if (target) {
      ...
      resetProjectToStarter(
        fs,
        projectId,
        seedFilesForStarter(source, resolveStarter(source, target.starter), root),
      );
