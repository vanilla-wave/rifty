...
export function wirePreviewBridge(
  ...
  try {
    ...
    throw new AggregateError(
      ...
      `preview bridge setup failed: ${[primary, ...cleanupErrors]
...
export interface WorkspaceOwnerHandle {
  ...
  /** Owner-authoritative backend, populated before `ready` resolves. */
  storageBackend(): 'opfs' | 'memory' | null;
  ...
  /**
   * Send one structured-clone-safe host-extension message after owner readiness.
   * Rejects if the owner exits or its fork-IPC channel refuses the send.
   */
  sendRawMessage(message: unknown): Promise<void>;
  /** Subscribe to owner messages not claimed by the package core. */
...
function isWorkspaceOwnerReadyMessage(message: unknown): message is WorkspaceOwnerReadyMessage {
  ...
  return (
    ...
    ((candidate as { readonly backend?: unknown }).backend === 'opfs' ||
      (candidate as { readonly backend?: unknown }).backend === 'memory')
  );
}
...
export function startWorkspaceOwner(opts: WorkspaceOwnerOptions): WorkspaceOwnerHandle {
  ...
  const root = validateWorkspaceRoot(opts.root ?? '/scratch', 'workspace owner root');
  ...
  const workspaceId = validateWorkspaceId(
    opts.workspaceId ?? createPreviewOwnerToken(),
  ...
  const rawMessageListeners = new Set<(message: unknown) => void>();
  ...
  let storageBackend: 'opfs' | 'memory' | null = null;
  ...
  const client = createPtyClient({
    ...
    onDevServer: (frame) => {
      notifySubscribers(devServerListeners, frame);
    },
    onPreview: (frame) => {
      notifySubscribers(previewListeners, frame);
    },
  });
  ...
  worker.on('message', (message: unknown) => {
    ...
    notifySubscribers(rawMessageListeners, message);
  });
  ...
  const { archiveBridge, fileReadBridge } = (() => {
    ...
    try {
      ...
      throw new AggregateError(
        [primary, ...cleanupErrors],
        `workspace owner setup failed: ${[primary, ...cleanupErrors]
          .map((failure) => failure.message)
          .join('; ')}`,
      );
    }
  })();
  let pageBridgesDisposed = false;
  const disposePageBridges = (): void => {
    if (pageBridgesDisposed) return;
    pageBridgesDisposed = true;
    runCleanupSteps(
      [() => archiveBridge.dispose(), () => fileReadBridge.dispose()],
      'workspace owner page bridge dispose failed',
    );
  };
  ...
  worker.on('exit', (code?: unknown) => {
    ...
    notifySubscribers(devServerListeners, stopped);
    ...
    notifySubscribers(previewListeners, noPreviews);
  ...
  return {
    ...
    async sendRawMessage(message) {
      await ready;
      if (exited) throw new Error('workspace owner has exited — raw message not sent');
      if (!worker.send(message)) {
        throw new Error('workspace owner fork-IPC channel refused raw message');
      }
    },
    onRawMessage(cb) {
      rawMessageListeners.add(cb);
      return () => rawMessageListeners.delete(cb);
    },
    close() {
