...
describe('OwnerRpcFs', () => {
  ...
  it('rejects a reflected explorer mutation when the owner durability drain fails', async () => {
    const fs = rpc();
    writer.flushDurable = () => Promise.reject(new Error('OPFS quota exhausted'));
    const created = fs.createFile('/workspace/src/quota.txt');
    publish();
    await expect(created).rejects.toThrow('OPFS quota exhausted');
  });
});
