/**
 * Wire an `npm` builtin into the Workbench shell. Installs run through
 * `@riftydev/npm-client.install` with the owner registry, VFS, and fetcher.
 *
 ...
 *
 * Auto-creates a minimal `package.json` at `ctx.cwd` when none exists so
 * `npm install express` is a one-liner on a blank tree.
 *
...
export interface NpmShellCommandDeps {
  ...
  readonly vfs: Vfs;
  /** Host-configured registry client. */
...
/**
 * Build the `npm` shell command. Workbench composition registers it on a
 * `ShellSession`; this factory stays framework-free.
 */
...
export async function executeNpmInstallOperation(
  ...
  if (nothingToInstall) {
    // This exact manifest needs no dependency tree. Preserve npm's no-op while
    // carrying the empty-plan proof to child admission; do not invent a tree,
    // lockfile, or install stamp.
