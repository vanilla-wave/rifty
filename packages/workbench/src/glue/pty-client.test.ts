...
describe('pty-client', () => {
  ...
  it('forwards a live resize for the correlated foreground run', () => {
    const { client, sent } = harness();

    client.resize('s1', 'r7', 132, 43);

    expect(sent).toEqual([{ type: 'pty:resize', sid: 's1', rid: 'r7', cols: 132, rows: 43 }]);
  });
