...
export function buildChildSpawnSpec(
  ...
  return {
    ...
    env: {
      ...
      ...(req.isTTY && req.cols !== undefined ? { RIFTY_TTY_COLS: String(req.cols) } : {}),
      ...(req.isTTY && req.rows !== undefined ? { RIFTY_TTY_ROWS: String(req.rows) } : {}),
    },
    cwd: req.cwd,
    serve: true,
  };
  ...
  return createBinExecutor({
    ...
    spawn: (req): BinWorkerHandle => {
      const handle = globalProcessManager.spawnWorker(
        req.shimPath,
        buildChildSpawnSpec(req, nodeEntryUrl, childConfig),
        1,
      );
