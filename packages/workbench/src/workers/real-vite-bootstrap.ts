...
import { amendStarterGeneratedBaseline, ensureStarterInitialCommit } from '../glue/starter.ts';
...
import { setActiveProjectReady } from './owner-project-readiness.ts';
...
import {
  type OwnerWorkerConfig,
  type WorkbenchStorageBackend,
...
function seedStarterBaseline(
  ...
  for (const [path, content] of Object.entries(
    seedFilesForStarter(catalog, resolveStarter(catalog, starter), root),
  )) {
    const np = normalizePath(path);
    fs.mkdirSync(dirname(np), { recursive: true });
    fs.writeFileSync(np, enc.encode(content));
...
async function bootShellOwner(opts: {
  ...
  readonly workerConfig: OwnerWorkerConfig;
  ...
  function primeInstallPrefetch(): void {
    const resolverUrl = getResolverUrl(workerConfig.registry.resolverUrl);
    ...
    const decision = decideInstallPrefetch({
      ...
      pinFor: () =>
        learnedPinForPackageJsonSync(syncMirror(), devCfg.packageJson) ??
        workerConfig.registry.resolverPins?.[devSpec.id],
    });
    ...
    installPrefetch =
      decision.kind === 'start'
        ? startInstallPrefetch({
            ...
            bundleBaseUrl: getEddyBundleBaseUrl(workerConfig.registry.resolverBundleUrl),
          })
        : undefined;
  }
  ...
  if (hiddenEmptyBoot) {
    ...
    seedProject(cfg);
    if (freshRoot) seedStarterBaseline(workerConfig.catalog, starter, cfg.root);
  ...
  const devServer = createDevServerController({
    ...
    boot: async (signal, devLog, devSid, terminal) => {
      ...
      // The owner store is what the shell and host worker extensions read. Restore/clean may replace
  ...
  const registry = createProxiedRegistryClient({
    registryUrl: workerConfig.registry.registryUrl,
  });
  let binRunSeq = 0;
  ...
  const childRuntimeConfig = {
    ...workerConfig.assets,
    kernelWorkerUrl: workerConfig.workers.kernelWorkerUrl,
    nodeEntryWorkerUrl: workerConfig.workers.nodeEntryWorkerUrl,
  };
  configureNodeEntryWorker(
    workerConfig.workers.nodeEntryWorkerUrl,
    buildChildRuntimeEnv(childRuntimeConfig),
  );
  ...
  const devServerChild = createOwnerChildDevServer(
    workerConfig.workers.devServerWorkerUrl,
    childRuntimeConfig,
  );
  ...
  const makeShell = (
    ...
    const npmCommand = createNpmShellCommand({
      ...
      resolverUrl: getResolverUrl(workerConfig.registry.resolverUrl),
      ...
      resolverClosureHash: () => workerConfig.registry.resolverPins?.[devSpec.id],
      resolverBundleBaseUrl: getEddyBundleBaseUrl(workerConfig.registry.resolverBundleUrl),
  ...
  const server = createPtyServer({
    ...
    onDevConfig: (config) => {
      devSpec = resolveProjectSpec(workerConfig.catalog, config.templateId);
      ...
      setActiveProjectReady(devConfigReady);
    },
  ...
  kernelIpc.send?.({ type: 'rifty:workspace-owner-ready', port, backend: opts.backend });
...
async function bootstrap(): Promise<void> {
  ...
  const workerConfig = parseOwnerWorkerConfig(env);
  installSqliteWasmSyncProvider(workerConfig.assets.sqliteWasmUrl);
  configureEsbuildWasmUrl(workerConfig.assets.esbuildWasmUrl);
  ...
  const spec = resolveProjectSpec(
    workerConfig.catalog,
    env.RIFTY_RFV_TEMPLATE ?? workerConfig.catalog.defaultTemplateId,
  );
  ...
  const starter = env.RIFTY_RFV_STARTER ?? workerConfig.catalog.defaultStarterId;
  ...
  let backend: WorkbenchStorageBackend = 'memory';
  try {
  ...
  setKernelWorkerUrl(workerConfig.workers.kernelWorkerUrl);
