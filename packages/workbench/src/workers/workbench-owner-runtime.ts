  ...
  try {
    ...
    const requireActiveProjectRoot = (): string => {
      if (activeProjectRoot === null) {
        throw new Error('Workbench owner execSync has no active project root');
      }
      return activeProjectRoot;
    };
    installOwnerSyncRuntimeHandlers(getKernelDispatcher(), () => authority, {
      mutationGuard: createWorkbenchOwnerChildVfsMutationGuard({
        activeProject: () =>
