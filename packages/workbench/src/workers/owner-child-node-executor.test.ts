...
describe('owner-child-node-executor', () => {
  it('builds a serve:true spec with RIFTY_BIN=0 and host-injected WASM assets', () => {
    ...
    expect(spec).toMatchObject({
      ...
      env: {
        ...
        RIFTY_ESBUILD_WASM_URL: 'blob:esbuild',
  ...
  it('streams stdout, reports listening, resolves on exit + removes', async () => {
    ...
    const onExit = vi.fn();
    const exec = createOwnerChildNodeExecutor('URL', assets, () => fake.h);
    const stdout: string[] = [];
