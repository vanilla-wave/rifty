...
function isRootDestructive(intent: VfsMutationIntent): boolean {
  switch (intent.kind) {
    ...
    case 'replace':
      // `replace` classifies an all-target transaction for the owner guard; it
      // is not an instruction to remove the directory. The namespaced FsSync
      // separately rejects physical root deletion/replacement operations.
      return false;
    case 'rename':
    case 'copy':
