...
export type NodeWorkerRuntimeEnv = NodeEntryHostRuntime & Readonly<Record<string, string>>;
...
function required(
  env: Readonly<{
    RIFTY_KERNEL_WORKER_URL?: string;
    RIFTY_NODE_ENTRY_WORKER_URL?: string;
    RIFTY_SQLITE_WASM_URL?: string;
  }>,
  key: 'RIFTY_KERNEL_WORKER_URL' | 'RIFTY_NODE_ENTRY_WORKER_URL' | 'RIFTY_SQLITE_WASM_URL',
  owner: string,
): string {
  const value = env[key];
  if (typeof value !== 'string' || value.length === 0) {
...
export function buildNodeWorkerRuntimeEnv(config: NodeWorkerRuntimeConfig): NodeWorkerRuntimeEnv {
