...
import type { Shell, StdinReader, TerminalResizeSource, TerminalSize } from '@riftydev/shell';
...
interface RunState {
  ...
  readonly terminal: RunTerminalSize;
  seq: number;
}
...
function validDimension(value: number, name: 'cols' | 'rows'): number {
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new RangeError(`pty ${name} must be a positive integer; received ${value}`);
  }
  return value;
}

/** One active run's mutable terminal grid + subscription chokepoint. */
class RunTerminalSize implements TerminalResizeSource {
  #size: TerminalSize;
  readonly #listeners = new Set<(size: TerminalSize) => void>();

  constructor(cols: number, rows: number) {
    this.#size = {
      cols: validDimension(cols, 'cols'),
      rows: validDimension(rows, 'rows'),
    };
  }
...
export function createPtyServer(deps: PtyServerDeps): PtyServer {
  ...
  async function exec(
    ...
    let terminal: RunTerminalSize;
    try {
      terminal = new RunTerminalSize(frame.cols, frame.rows);
    } catch (err) {
      deps.send({
        type: 'pty:exit',
        sid,
  ...
  function handleFrame(frame: PageToOwnerFrame): void | Promise<void> {
    switch (frame.type) {
      ...
      case 'pty:resize': {
        sessions.get(frame.sid)?.runs.get(frame.rid)?.terminal.resize(frame.cols, frame.rows);
        return;
      }
      case 'pty:close': {
