...
export interface DevServerChildSpawnParams {
  ...
  /** Foreground terminal grid at spawn time. */
  readonly terminalSize?: TerminalSize;
}

export type RecursiveWorkerConfig = WorkbenchChildRuntimeConfig;
...
export function buildDevServerChildSpawnSpec(
  ...
  workerConfig: RecursiveWorkerConfig,
): SpawnWorkerSpec {
  return {
    ...
    env: {
      RIFTY_REMOTE_FS: '1',
      ...buildChildRuntimeEnv(workerConfig, params.projectSpec),
      ...
      ...(params.terminalSize
        ? {
            ...
            RIFTY_TTY_COLS: String(params.terminalSize.cols),
            RIFTY_TTY_ROWS: String(params.terminalSize.rows),
          }
        : {}),
    },
    cwd: params.root,
      ...
      return new Promise<DevServerHandle>((resolve, reject) => {
        const handle = spawn(
          buildDevServerChildSpawnSpec(
            {
              ...opts.params,
              ...(opts.terminal ? { terminalSize: opts.terminal.current() } : {}),
            },
            devServerWorkerUrl,
            workerConfig,
          ),
        );
        const unsubscribeResize = opts.terminal?.subscribe(({ cols, rows }) => {
          handle.resize(cols, rows);
        });
        let resizeClosed = false;
        const closeResize = (): void => {
          if (resizeClosed) return;
          resizeClosed = true;
          unsubscribeResize?.();
        };
        ...
        handle.on('message', (message: unknown) => {
          ...
          } else if (m.type === 'rifty:dev-error') {
            closeResize();
