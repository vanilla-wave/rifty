...
const assets = installChildRuntimeConfig(proc.env);
...
// correct argv/cwd/stdin + fork-IPC `send`. No swap, so the stdin capability guard and
...
if (proc.env.RIFTY_NODE_SERVE === '1') {
  ...
  // ADR-0230: the owner forwards flowing stdin; keep unsupported pull/raw
  // surfaces loud instead of degrading to TypeError or false-success no-ops.
  installStdinCapabilityGuards(proc);
