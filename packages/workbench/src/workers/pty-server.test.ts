...
describe('pty-server', () => {
  ...
  it('pushes a mid-run resize through the shell terminal source', async () => {
    const out: OwnerToPageFrame[] = [];
    ...
    shell.registerCommand('watch-size', async (_args, ctx) => {
      ...
      if (!terminal) throw new Error('missing terminal resize source');
      sizes.push(terminal.current());
      const unsubscribe = terminal.subscribe((size) => sizes.push(size));
      await release.promise;
      unsubscribe();
      return 0;
    });
    const server = createPtyServer({ send: (frame) => out.push(frame), makeShell: () => shell });
    server.handleFrame({ type: 'pty:open', sid: 's1' });
    const run = server.handleFrame({
      type: 'pty:exec',
      sid: 's1',
    ...
    server.handleFrame({ type: 'pty:resize', sid: 's1', rid: 'r1', cols: 132, rows: 43 });
    ...
    release.resolve();
    await run;
    expect(out.some((frame) => frame.type === 'pty:exit' && frame.code === 0)).toBe(true);
  });
