import { createPreviewRouteSet } from '../glue/preview-route-set.ts';
/**
 ...
 *
 * No UI or reactive-framework imports; every side effect goes through the
 * injected ports below, which are also the behavioral-test seam.
 */
import type { PreviewPortEntry, PtyDevServer, PtyPreview } from '../glue/pty-protocol.ts';
import { createSnapshotObservable } from '../observable.ts';
...
export interface DevServerLifecycleDeps<S extends DevServerSessionLike> {
  ...
  /** Boot lines for a starter at the active root. */
  bootLines(starter: WorkbenchStarter): readonly string[];
  /** Store-derived active starter — restart boots this, never stale picker state. */
  activeStarter(): WorkbenchStarter;
  projectSpecForStarter(starter: WorkbenchStarter): ProjectSpec;
...
export interface DevServerLifecycleSnapshot {
  ...
  readonly previewPorts: readonly PreviewPortEntry[];
  readonly sessionId: string | null;
  readonly stoppableSessionId: string | null;
...
export interface DevServerLifecycle<S extends DevServerSessionLike> {
   ...
   * new owner. Owner rebinding stays explicit so hosts control its lifetime.
   */
  attachOwner(owner: DevServerOwnerPort): void;
  /** Unsubscribe from the owner and tear every live preview bridge. */
  dispose(): void;
  /** Atomic framework-free state read for host UI bindings. */
  snapshot(): DevServerLifecycleSnapshot;
  /** Observe state changes; read `snapshot()` once before subscribing. */
  subscribe(listener: (snapshot: DevServerLifecycleSnapshot) => void): () => void;
  status(): 'stopped' | 'starting' | 'running';
  running(): boolean;
  previewPorts(): readonly PreviewPortEntry[];
...
function throwTeardownErrors(errors: readonly Error[]): void {
...
export function createDevServerLifecycle<S extends DevServerSessionLike>(
  ...
  const { terminal } = deps;
  let devServerStatus: 'stopped' | 'starting' | 'running' = 'stopped';
  ...
  let previewPorts: readonly PreviewPortEntry[] = [];
  ...
  function readSnapshot(): DevServerLifecycleSnapshot {
    return {
      ...
      generation: devServerRestartGeneration,
    };
  }

  const observable = createSnapshotObservable(readSnapshot);
  ...
  const previewRoutes = createPreviewRouteSet({ mountBridge: deps.wirePreviewBridge });
  function reconcileBridges(): void {
    const owner = currentOwner;
    if (!owner) return;
    previewRoutes.reconcile(owner.previewOwnerToken, previewPorts);
  }

  function tearDownOwner(disposeRoutes: boolean): Error[] {
    const errors: Error[] = [];
    for (const unsubscribe of [unsubscribeDevServer, unsubscribePreview]) {
      try {
        unsubscribe?.();
      } catch (error) {
    ...
    currentOwner = null;
    try {
      if (disposeRoutes) previewRoutes.dispose();
      else previewRoutes.clear();
    } catch (error) {
      errors.push(asError(error));
    }
    return errors;
  ...
  function attachOwner(owner: DevServerOwnerPort): void {
    throwTeardownErrors(tearDownOwner(false));
    currentOwner = owner;
    try {
      ...
      const errors = [asError(error), ...tearDownOwner(false)];
      throw new AggregateError(errors, errors.map((failure) => failure.message).join('; '));
    }
  }

  function dispose(): void {
    try {
      throwTeardownErrors(tearDownOwner(true));
    } finally {
      observable.dispose();
    }
  }
  ...
  function clearDevServerBootSession(sessionId: string): void {
    if (devServerBootSessionId === sessionId) {
      devServerBootSessionId = null;
      observable.notify();
    }
  }
  ...
  async function stopSession(sessionId: string | null): Promise<void> {
    ...
    if (stopLifecycleRun) {
      ...
      // No local preview-port wipe: the set is OWNER-derived (pty:preview) —
  ...
  async function startSession(
    ...
    void deps.runBootSequence(targetSessionId, bootLinesOverride ?? deps.bootLines(starter));
    // runBootSequence synchronously marks the terminal running before its first
    // await; publish after dispatch so lifecycleRunning is truthful for CLIs too.
    observable.notify();
    return waitForPresetBoot(targetSessionId, generation, deps.projectSpecForStarter(starter));
  }
  ...
  async function restart(sessionId: string): Promise<void> {
    ...
    await startSession(sessionId, generation, deps.activeStarter());
  }
  ...
  return {
    ...
    nextGeneration: () => {
      devServerRestartGeneration += 1;
      observable.notify();
      return devServerRestartGeneration;
    },
