...
- **A live kernel-backed `worker_threads.Worker` keeps the spawning realm's event loop alive (Node parity).** The kernel `Worker` did not participate in the run-to-completion keepalive (`ref()`/`unref()` were no-ops), so a `node <file>` program driving worker_threads could drain-reap before its worker finished. Now a live kernel worker holds one keepalive ref (acquired on spawn, released on exit/terminate); `ref()`/`unref()` toggle it like Node (default refed). The same-realm fallback is unaffected. Surfaced while scoping the Rolldown-WASI production-build path (`backlog/playground/vite8-production-build-preview`); lands standalone as a fidelity fix.
