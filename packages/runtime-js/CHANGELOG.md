...
- Node process lifecycle (ADR-0445, `builtins/process-lifecycle-events.ts`): uncaught errors (timers, `setImmediate`, fs/zlib/`util.callbackify` callbacks, `nextTick`, the entry) and unhandled rejections reach `uncaughtExceptionMonitor`/`uncaughtException`/`unhandledRejection` listeners before any terminal path, with Node's origins and `UnhandledPromiseRejection` wrap; no listener → `'exit'` 1 then the existing loud path, a rejection's stderr + exit request (`exitCode ?? 1`) at once so a later `exit()` never overrides it; a throwing listener → stderr + status 7. `exit()` follows Node: `'exit'` once, no argument = `exitCode`, status read after the listeners, one kernel exit request. `NodeProcess.exitCode` is `number | undefined` (unset reads `undefined`); new `resetNodeProcessExit(process)` on `./builtins/process`. `awaitDrain` settles one host task after the first zero-ref sample, so a late Chromium `unhandledrejection` is never a silent exit 0.
