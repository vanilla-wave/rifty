 ...
 * `syncRing` is shared; the four fixed `stdio`/IPC ports and URL-entry
 * capability ports MUST appear in the parent's `postMessage` transfer list.
 */
...
function closeCapabilityPorts(entry: WorkerEntryDescriptor): void {
  ...
  try {
    keys = Reflect.ownKeys(entry.capabilityPorts);
  } catch {
    return;
  }
  for (const key of keys) {
    try {
      const descriptor = Object.getOwnPropertyDescriptor(entry.capabilityPorts, key);
      if (descriptor === undefined || !('value' in descriptor)) continue;
      const value = descriptor.value as { close?: unknown } | null;
      if (typeof value !== 'object' || value === null || closed.has(value)) continue;
      closed.add(value);
      if (typeof value.close === 'function') value.close();
    } catch {
      /* malformed/already-closed child endpoint */
    }
  }
}
  ...
  const onMessage = async (ev: MessageEvent): Promise<void> => {
    ...
    const outcome = await runEntryLifecycleInternal(spec, {
      prepareEntry: (workerSpec) => {
        ...
        // Expose sync/process state before entry-scoped bootstrap/capabilities;
        // one runEntryLifecycle guard catches every setup boundary.
