...
| **data: URL ESM import** | `import('data:text/javascript,…')` / `data:application/json` · v12.10 | `resolver.ts:129` throws `UNSUPPORTED_PROTOCOL`. Parse mediatype, `;base64`→atob else percent-decode + TextDecoder, route through existing `executeEsm` esm/json path. No network. (file:// is the higher-value sibling, already backlogged separately.) | low — mostly test frameworks/codegen/inline-worker bootstraps | M | low |
...
- **file:// ESM import** — higher-value sibling of `data:` URL import; already backlogged separately (`loader-file-url-resolution.md`), so excluded here despite being adjacent to candidate in §6.
