...
> TL;DR: `tokenize` emits `Token{value,quoted}` not `string[]`; dispatcher does single-segment glob (`* ? [...]`) on unquoted words via the hand-rolled vfs-grep walker, pass-literal on no-match, `**`/braces deferred to picomatch
