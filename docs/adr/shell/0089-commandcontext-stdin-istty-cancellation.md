...
> TL;DR: add 4 optional `CommandContext` fields (stdin reader, isTTY+cols/rows, AbortSignal) + `RunOptions.signal` so pipe/redirect, TTY-gated color, and SIGINT-resolves-run land additively without breaking registrants
