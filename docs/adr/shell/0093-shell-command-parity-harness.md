...
> TL;DR: per-command-class parity: reuse node-parity-runner for node:fs-expressible filters (live Node gold), else frozen pinned-GNU golden fixtures labeled conformance-not-parity; reject a live host-coreutils spawn oracle; unimplemented flags throw + register ❌
