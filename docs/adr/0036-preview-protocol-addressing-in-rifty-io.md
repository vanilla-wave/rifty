...
- `@riftydev/service-worker` owns the SW-side intercept. Its
...
- `@riftydev/net` documents the convention in `registry.ts:4` ("the SW
...
A new module `@riftydev/io/src/preview-protocol.ts` owns the addressing
primitives. Both `@riftydev/service-worker` and `@riftydev/net` import from
...
Re-exported from `@riftydev/io/src/index.ts` so callers reach the surface
through the package's main entrypoint.

The implementation lives in `@riftydev/io` because:
...
- `service-worker/package.json` currently declares zero workspace deps
  (the SW bundler follows TS imports). Adding `@riftydev/io` is the
...
#### Option A: a new package `@riftydev/preview-protocol`
...
#### Option B: co-locate in `@riftydev/net`
...
- `service-worker/package.json` gains `"@riftydev/io": "workspace:*"` (was
...
- `packages/service-worker/src/preview-bridge.ts` drops its inline
  ...
  `parsePreviewPath` from `@riftydev/io`. `matchPreviewUrl` remains as the
...
- `packages/net/src/registry.ts` doc comment points at
  `@riftydev/io/preview-protocol` as the canonical reference for the URL
  scheme.
...
- [x] `pnpm typecheck` / `pnpm lint` / `pnpm check:deps` / `pnpm test:run`
      clean.
- [x] CHANGELOG entries in `@riftydev/io`, `@riftydev/service-worker`,
      `@riftydev/net`.
