...
1. **Addressing scheme.** The `/preview/<port>/...` URL convention and preview Host synthesis live in `@riftydev/io/preview-protocol` (ADR-0036, corrected 2026-07-04 by ADR-0189 D3): `PREVIEW_PREFIX_RE`, `PREVIEW_LOCAL_HOST`, `synthesizePreviewUrl(path, port?)`. Renaming the synthesized Host or reshaping the regex changes behaviour silently — `SW_PROTOCOL_VERSION` covers frame fields, not the URL contract.
