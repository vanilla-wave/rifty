...
- `docs/backlog/playground/editor-same-path-write-serialization.md` — completed; `createPlaygroundDocumentWriter` owns one FIFO per path, and `playground-project-view.contract.test.ts` pins concurrent writes whose first save completes last. The item's cited `editor-host-core.ts` was never the serialization owner
