...
| 0189 wrapper-retirement backlog-open clause | 0189 note 2026-07-07 | Vite CLI wrapper/argv/env branch deleted; template gates moved to visible `vite.config.js`; `net/preview-websocket-bridge` + `preset-deglue` removed delete-on-done |
| 0047 `build()`-via-`runWasi`-or-throw clause | 0192 / note 2026-07-10 | guest esbuild JS API (`transform`/`build`/`context`) delegates to the host esbuild-wasm bridge; ADR-0047's WASI binary stays the CLI-conformance forcing consumer |
...
| 0078 `ServerSpec` / `hmrEnabled` bootstrap fields | 0078 note 2026-07-07 | template-specific Vite policy moved to visible seeded `vite.config.js`; worker no longer reconstructs hidden server/optimizer/HMR knobs |
...
- `docs/backlog/playground/react-preset-dev-boot-gaps.md` — ai-mode branch item removed on close (ported ahead of PR #111); the record is ADR-0192 + the code (ADR-0192 still cites it)
- `docs/backlog/playground/honest-vite-command-umbrella.md` — completed backlog item, removed on close; the record is ADR-0174 + the code (ADR-0174 still cites it)
...
- `docs/backlog/net/cross-realm-listen-eaddrinuse.md` — completed backlog item, removed on close; the record is ADR-0186 + the code (ADR-0180 D5 still cites it)
