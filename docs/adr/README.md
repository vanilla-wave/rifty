...
- `docs/backlog/playground/preset-switch-port-flip-window.md` — retired after the Workbench migration; switching now closes the full old session before opening the next, so keeping the old preview live is not the accepted behavior (ADR-0130/0165 still cite the historical item)
- `docs/backlog/playground/per-project-persistent-port.md` — retired after the Workbench migration; the old owner-respawn/eager-port-flip premise no longer applies and no current user-visible port-identity fault is reproduced (ADR-0165 still cites the historical item)
- `docs/backlog/playground/editor-same-path-write-serialization.md` — retired; per-path editor write serialization exists on current `main` (`createPlaygroundDocumentWriter` in `apps/playground/src/adapters/playground-project-view.ts` chains every editor write per path; CAS versions are the second layer). The item's cited `editor-host-core.ts` was never the serialization owner
