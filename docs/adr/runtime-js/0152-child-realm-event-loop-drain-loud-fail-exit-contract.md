...
> **Corrected (2026-09-25, ADR-0445):** `awaitDrain()` does not settle on the first zero-ref sample; it settles after the next host task confirms zero refs and no new rejection (Chromium queues `unhandledrejection` late). Handle set and poll shape unchanged.
...
> **Corrected (2026-09-25, ADR-0445):** the trap first delivers to Node listeners. A rejection an `unhandledRejection`/`uncaughtException` listener handles gets `preventDefault()` and is not recorded. With no listener: `'exit'` 1, then stderr and the kernel exit request `uint8(exitCode ?? 1)` in the trap; the drain records that exit signal. Loud default stands. A Node callback already queued before Chromium's `unhandledrejection` task: gap, backlog `runtime-js/late-unhandled-rejection-drain`.
