...
> TL;DR: export io's zero-copy `decode` as public `bytesToString`; rewrite fs.decodeResult encoded branch to it, dropping the throwaway `Buffer.from` copy; `!encoding` owned-mutable branch untouched
