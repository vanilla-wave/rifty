...
> TL;DR: `SabRing` returns zero-copy SAB views, dispatcher arms `waitAsync` on REQ_STATE (backstop timer + setInterval fallback), payloadCapacity is spec-configurable with exact-size guard, and SyncRpc bumps to v2 with a 1-byte JSON/BINARY frame discriminator so execSync returns byte-exact stdout
