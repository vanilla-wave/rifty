...
> TL;DR: add non-throwing `statSyncOrNull` to shared `FsSync` (both backends), collapsing 7 resolver existsSync+statSync double-probes to one probe each; `statSync` stays throwing
