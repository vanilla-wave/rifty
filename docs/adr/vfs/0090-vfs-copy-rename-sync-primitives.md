...
> TL;DR: add `copyFileSync`/`cpSync`/`renameSync` to `FsSync` (both backends), node:fs-Sync-faithful, mtime-preserving rename + best-effort fail-fast recursive copy, so shell cp/mv stop hand-rolling read+write+rm
