...
> **Corrected (2026-06-20):** `createOwnerChildNodeExecutor` **mirrors** `bin-executor.ts`'s foreground machinery (decodeChunk / stream / SIGINT-kill→130 / exit-code) **but does NOT call `createBinExecutor`** — it reimplements them inline. The listening-server case needs a wider seam than the `BinExecutor` contract (`(binPath,args,ctx)=>Promise<number>`) exposes: a child→owner `rifty:node-listening` `on('message')`→`onListening` hook and an `onExit` registry-remove, neither of which fit that signature; the `'exit'` listener is also registered BEFORE the pre-abort `kill` (kill() emits `'exit'` synchronously) to keep a pre-aborted run from hanging — the reverse of `createBinExecutor`'s order. Only `createOwnerChildBinExecutor` reuses `createBinExecutor` verbatim. Shared-driver extraction (`runForegroundChild`) is `backlog: playground/owner-child-foreground-shared-driver`.
...
> **Corrected (2026-06-20, ADR-0157):** the bootstrap no longer SWAPS `globalThis.process` (the original code installed a minimal seeded shim at pre-entry, then re-swapped to the default `riftyProcess` via `installRuntimeGlobals`, orphaning argv/cwd/stdin). ADR-0157 installs ONE spec-seeded mutable process at the pre-entry seam, so the serve branch reads the same faithful process user code sees. Also: the claim that "the `.bin`/`execSync` path is unchanged" held for stdio but NOT for `Buffer`/`process.nextTick` — that path silently lacked both (a latent fidelity gap); ADR-0157 gives every Node worker `Buffer` + nextTick-ordering by construction. The keepalive/drain framing in this section is unchanged.
