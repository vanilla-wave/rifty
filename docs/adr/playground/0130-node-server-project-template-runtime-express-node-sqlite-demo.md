...
D2 — **Worker bootstrap branch, common head.** Runtime globals / VFS bridges / seed / npm install / loader / `setProcessCwd(root)` (both runtimes agree with the project root; servers resolve `express.static('public')` from cwd) stay shared; tail dispatches on `cfg.runtime`. Node branch: optional sqlite engine init, `loader.import(entryPath)`, loud `waitForListeningPort` (no silent dead preview). HMR bridge + esbuild/rollup shims stay vite-only.
