...
- A reload during a boot-restore's drain window finds no stamp → arrival re-runs (correct, visible, idempotent). *(Corrected 2026-07-04: FIFO order rules out a stamp landing BEFORE the tree, but a swallowed per-op persist failure could still stamp a torn tree on the boot path — closed round 14 by the deferred durability check: a dirty post-drain ledger revokes the stamp.)*
