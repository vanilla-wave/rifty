...
6. **Explorer file management is WRITABLE too (corrected 2026-06-14).** A sandbox whose tree you cannot touch is pointless, so the real-vite explorer is no longer read-only. `glue/real-vite-explorer-vfs.ts`'s `RealViteExplorerVfs` is one `FsOpsTarget` that READS the snapshot (tree reflects the worker, incl. worker-seeded `index.html`) but on every mutation WRITES the page mirror AND propagates a frame to the worker: `writeFileSync`→`write`, `mkdirSync`→`mkdir`, `rmSync`→`rm`. The write port (`vfs-write-port.ts`) gains an **`rm` frame** (delete + rename's remove leg) alongside `write`/`mkdir`; `RealViteHandle.applyVfsFrame(frame)` pushes any frame (IPC fast-path, BroadcastChannel fallback). `readOnly = false`, so `FileExplorer` shows its CRUD controls and no badge; `fs-ops` (recursive `copyTree` rename, collision guards) runs unchanged on top. `node_modules` rows stay read-only (the row actions are gated on `!isUnderNodeModules`) — installed packages are worker-owned. The worker applies each frame and republishes the snapshot, so the change appears in the read view (one round-trip; the explorer poll reconciles).
...
- (+) Explorer **file management** (new / rename / delete) works in real-vite via `RealViteExplorerVfs` + the write port's additive `rm` frame; each op round-trips through the worker and reappears in the snapshot. No more `read-only` badge. Covered by `glue/real-vite-explorer-vfs.test.ts` + `glue/vfs-write-port.test.ts`.
...
- (=) Write port stays one-way page→worker; the `rm` frame is additive to `write`/`mkdir`.
- (+) Honest read-only for worker-OWNED files: `node_modules` (explorer rows gated on `!isUnderNodeModules`) + worker-generated output open view-only — the page never writes them back.
