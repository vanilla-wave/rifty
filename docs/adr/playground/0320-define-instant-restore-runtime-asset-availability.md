...
2. Vite 7.3.6 with esbuild 0.28.0 and an empty verified shadow CAS requests
   ...
   runtime bytes.
3. Cold offline absence fails admission with `ShadowAssetError`,
   `code: 'ESHADOWASSET'`, `phase: 'acquire'`. A verified warm persistent CAS
   serves later owner boots with zero registry requests.
...
- [browser companion pins the exact cold Vite 7 requests and Vite 8 empty plan](../../../tests/browser-unit/workbench-playground-companion.spec.ts);
...
|---|---|
| `provenance-lie` × instant Vite 7 with empty CAS and offline registry | project admission rejects as `ESHADOWASSET` / `acquire`; no ready state or fallback |
