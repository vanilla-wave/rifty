...
- **Enabling cross-links (this strategy is inert without them):** ADR-0082 (CommandContext stdin/isTTY/cancellation — gates stdin-filter modes, TTY-gated color, long-running vite/node), ADR-0084 (tokenizer rich-token-type + single-segment glob — `* ? [...]` expansion with quote-provenance), ADR-0086 (shell-command parity harness — node-parity-runner has no `ls`/`grep`/`find` oracle, so the parity claim is unsubstantiated until 0086 lands). ADR-0083 (VFS cp/mv primitives) gates cp/mv; ADR-0085 records the git contract.
...
**REVERSIBLE** at the per-command level — each new pure-JS builtin that uses only the *existing* CommandContext is a zero-dep, no-public-API-change addition needing no ADR (reverting one is a few lines in `packages/shell`). A builtin requiring a *new* CommandContext field is gated by ADR-0082; one requiring a new VFS primitive by ADR-0083; the tokenizer/glob change by ADR-0084.
