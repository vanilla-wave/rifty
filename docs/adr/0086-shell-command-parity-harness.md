...
- **Future WASI path (ADR-0081 Option B, uutils):** uutils `ls`/`find`/etc. parity uses this SAME chosen oracle (node:fs reuse where applicable, else the frozen GNU fixture) BEFORE rifty relies on the WASM applet — the cutover (today's flagless `ls` → uutils honoring `-la --color`) is gated on green fixtures, updated without weakening (research §4 B, §9).
...
- [ ] For the future WASI/uutils path (ADR-0081 Option B), `ls`/`find`/etc. pass against this chosen oracle before rifty routes to the WASM applet.
