...
Alternatives rejected: POST-302→GET (extra RTT every cold resolve); client-persisted dep-set→hash map (whenever it would exist, an install stamp already suppresses the install) — *revisited by ADR-0194 (learned pins): measured 2026-07-02, a new project in the same profile has NO stamp yet, so the rejected premise was incomplete; ADR-0194 §8 records the implementation*; re-resolve-on-GET-miss (hash not invertible).
