...
5. **Durable-before-link**: a cold POST `await`s `store.put()` before writing the mutable link and responding (~150ms on a multi-second cold path). `put` is idempotent + self-healing — it re-seeds a missing OR corrupt/foreign object (`get` reads either as a miss; content-addressed, so `get` also rejects a bundle whose manifest hash ≠ the key) and skips the upload ONLY when the SAME bytes are already durable (S3 single-part ETag = body MD5). (**Corrected** from the original HEAD-`has()`-gate, which skipped the put whenever an object merely *existed* → a corrupt/truncated/foreign object was never overwritten and stayed a permanent GET-by-hash 404. Review follow-up.) A failed put logs loudly, skips the link, and still serves the computed bundle — degraded to recompute-next-time, never a 500. Origin state (mutable links, packument/tarball caches) is reconstructible RAM only → restarts/deploys lose nothing durable, hosts scale without shared disk or sticky routing.
