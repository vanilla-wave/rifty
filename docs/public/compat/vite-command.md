...
| `vite preview` (Vite 7 preset) | ⚠️ | Real CLI preview registers the production preview route and runs the user's `vite.config.*` — no rifty preview-config guard, no forced cors (both retired to execution-boundary honesty). Headers + same-realm/external proxy flow through the same-origin SW bridge; the Origin/isolation-header shape difference is a signposted platform limit (`vite-preview-origin-isolation-signpost`), and an unsupported preview op (e.g. a cross-realm proxy target) throws at its net execution boundary — never a pre-flight config guard. `tests/e2e/vite7-build-preview.spec.ts`. |
