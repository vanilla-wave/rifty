...
| `vite preview` (Vite 7 preset) | ⚠️ | Real CLI preview registers the production preview route; user preview config/CORS parity is not claimed because the browser wrapper currently forces the inline preview config. `tests/e2e/vite7-build-preview.spec.ts`. |
