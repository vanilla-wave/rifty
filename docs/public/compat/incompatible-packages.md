...
| `@esbuild/<platform>` | esbuild's platform binaries (optional deps of `esbuild`). | ⏭️ skipped; the installer materializes the `esbuild` entry from the shadow-registry shim at install time (ADR-0188) — backed by the WASI transform bridge. **Vite 8 transforms via oxc/Rolldown, not esbuild** — no esbuild shim lands in a Vite 8 tree. |
