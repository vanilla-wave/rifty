...
| `@esbuild/<platform>` | esbuild's platform binaries (optional deps of `esbuild`). | ⏭️ skipped; the installer materializes the `esbuild` entry from the shadow-registry shim at install time (ADR-0188), backed by the exact upstream-derived `esbuild-wasm@0.28.0` client over guest VFS. **Vite 8 transforms via oxc/Rolldown, not esbuild** — no esbuild shim lands in a Vite 8 tree. |
