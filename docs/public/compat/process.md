...
| Interactive stdin (`readline` / `process.stdin`) | ❌ | The session's stdin is NOT forwarded to the child; the seeded `process.stdin`'s consume surface (`on('data')`/`once`/`addListener`/`read`/`resume`/`pause`/`setEncoding`/`setRawMode`/`pipe`/async-iter) throws `NotImplementedError` — loud, never a silent hang (`'end'`/`isTTY` stay passive). `backlog/kernel/worker-per-process-residuals` (+ `backlog/terminal/raw-stdin-deferred-items`) |
