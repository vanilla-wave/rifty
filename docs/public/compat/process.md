...
| Child stdio and fork IPC | ⚠️ | Claimed `pipe`/`inherit`/`ignore`, explicit process stream targets, fork's single `ipc` slot, final drain, exit-before-close, default JSON and advanced structured-clone messages work. Advanced Date/Map/Set/RegExp/Error, standalone ArrayBuffer-backed typed arrays by type/bytes, nested BigInt and cycles are parity-proven; Buffer, plain-object accessors and unknown host-object payloads throw `NotImplementedError`, while raw SharedArrayBuffer gets Node's clone error. Numeric extra descriptors, unrestricted inheritance, handles, callbacks/options, and channel `ref()`/`unref()` remain loud gaps. |
| SAB-backed views in advanced fork IPC | ❌ | Node's V8 serializer copies them onto ordinary ArrayBuffers with observable offset/alias rules that native structured clone does not match. Rifty throws `NotImplementedError('child_process.serialization.advanced.shared-view')` before posting; it never exposes shared backing as a successful Node IPC message (ADR-0454). |
| Explicit ArrayBuffer/view alias in advanced fork IPC | ❌ | Node breaks this alias; native structured clone retains it and changes observable data after a backing write. Rifty throws `NotImplementedError('child_process.serialization.advanced.arraybuffer-view-alias')` before posting (ADR-0458). |
