...
| Node CLI CommonJS preload `--require`/`-r` | ❌ | Node preloads before eval/program execution. Valid Rifty forms throw `NotImplementedError('workbench.node.preload-context')` before allocation; incomplete options retain Node's exit-9 usage error. Tracked in `backlog/runtime-js/node-cli-preload-import-flags`. |
| Node CLI ESM preload `--import` | ❌ | Node imports before eval/program execution. Valid Rifty forms throw `NotImplementedError('workbench.node.preload-context')` before allocation; incomplete options retain Node's exit-9 usage error. Tracked in `backlog/runtime-js/node-cli-preload-import-flags`. |
