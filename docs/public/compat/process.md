...
| Node 24 TypeScript-stripping `node -e/-p` and explicit input types | ❌ | Source requiring TypeScript and explicit `commonjs-typescript`/`module-typescript` eval forms throw `NotImplementedError('runtime-js.node-eval-typescript-context')` before allocation. `module-typescript` print forms preserve `ERR_EVAL_ESM_CANNOT_PRINT`. Tracked in `backlog/runtime-js/node-cli-typescript-eval-context`. |
| Node CLI CommonJS preload `--require`/`-r` | ❌ | Node preloads before eval/program execution. Rifty reports the leading option as `node: bad option`, exits 9, and allocates no child; it never ignores the preload. Tracked in `backlog/runtime-js/node-cli-preload-import-flags`. |
| Node CLI ESM preload `--import` | ❌ | Node imports before eval/program execution. Rifty reports the leading option as `node: bad option`, exits 9, and allocates no child; it never ignores the preload. Tracked in `backlog/runtime-js/node-cli-preload-import-flags`. |
