...
`packages/workbench/src/workers/preview-registry.test.ts`. Run-to-completion loader parity (shebang,
...
| Flowing non-TTY stdin (`data` / `setEncoding`) | ✅ | The terminal→owner→supervised-child pump preserves chunk order for both `node <file>` and `.bin` children. `process.stdin` is non-TTY; `data` listeners, `setEncoding`, `resume`, and `pause` work. Source/destination faults terminate the child loudly. ADR-0230 |
| Pull/pipe/async/raw stdin + terminal EOF | ❌ | `readable`, `read`, `pipe`, async iteration, and `setRawMode` remain `NotImplementedError` ceilings. Ctrl+D/EOF line discipline is not claimed. Tracked by `backlog/kernel/worker-per-process-residuals`, `backlog/terminal/raw-stdin-deferred-items`, and `backlog/terminal/ctrl-d-eof-line-discipline`. |
| Live terminal resize / `SIGWINCH` | ✅ | A run-scoped `pty:resize` updates TTY dimensions, stream `resize`, and process `SIGWINCH` for the active foreground owner/child path; stale run ids cannot resize replacements. ADR-0225 |
