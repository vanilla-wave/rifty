...
| `node -e/-p` eval context | ❌ | Throws `NotImplementedError('workbench.node.eval-context')` rather than repeat the retired temporary-`.cjs` approximation (whose argv, `[eval]` filename/module identity, and `require.main` differed from Node 24). See `backlog/runtime-js/node-cli-eval-identity-parity`. |
