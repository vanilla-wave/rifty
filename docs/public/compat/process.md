...
| `node -p/--print[=rhs] -- <file>` program transition | ❌ | Node retains the exact print option in program `execArgv`; this CLI print context remains unsupported. Rifty throws `NotImplementedError('workbench.node.print-program-context')` before allocating a child rather than launch with false identity. An empty first token after `--` remains supported entryless eval. See `backlog/runtime-js/node-cli-print-program-context`. |
...
| Child stdio and fork IPC | ⚠️ | Claimed `pipe`/`inherit`/`ignore`, explicit process stream targets, fork's single `ipc` slot, final drain, exit-before-close, default JSON and advanced non-binary messages (including cycles and shared references) work. Binary serialized graphs (Buffer, typed arrays, DataView, ArrayBuffer/SharedArrayBuffer, including nested occurrences) throw `child_process.serialization.advanced.binary` before dispatch. QuickJS Proxy-backed object/array mirrors fail native cloning (ADR-0467). Numeric extra descriptors, unrestricted inheritance, handles, callbacks/options, and channel `ref()`/`unref()` throw loudly. |
...
| Binary advanced IPC graphs | ❌ | Buffer, typed arrays, DataView, ArrayBuffer/SharedArrayBuffer in the serialized graph throw `child_process.serialization.advanced.binary` before dispatch; channel remains usable (ADR-0467). |
| Public IPC vs private process control | ✅ | Plain spawn has no `process.send`; fork uses logical IPC with default JSON or explicit `serialization: 'advanced'`. Listening, port removal, physical exit, peer death, and subtree control stay private and remain live after public disconnect. |
