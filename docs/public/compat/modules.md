...
| CJS ↔ ESM interop (`require(ESM)`) | ⚠️ | Node 24 synchronous plain-JS graphs: shared live namespace, cached default/`__esModule` facade, `"module.exports"` override, transitive-TLA preflight (`ERR_REQUIRE_ASYNC_MODULE`), live ESM cycles, direct cross-format cycle errors with ordinary CJS partial back-edges preserved, and import-race errors (ADR-0346). Explicit `.mjs`, `"type":"module"` `.js`, and syntax-detected `.js` are eligible; extension/index fallback stays CJS-only as above. Async-transformed TS/TSX/JSX, Node CLI opt-out/trace flags, and public `require.cache` are outside this claim and stay loud/separately tracked. `process.features.require_module === true`. |
