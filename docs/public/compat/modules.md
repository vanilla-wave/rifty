...
| `worker_threads.Worker` `argv` / `name` / `resourceLimits` / `transferList` / `trackUnmanagedFds`; `env: null` | ❌ | The five options are ignored silently (Node: extra `process.argv`, `threadName`, heap limits, transfers) — `backlog/runtime-js/worker-threads-ignored-options`. `env: null` throws `TypeError` where Node starts the worker — `backlog/runtime-js/worker-threads-env-null` |
...
after Node's option validation: both engines contextify a given object and never hand out a realm's
own global (ADR-0464).
