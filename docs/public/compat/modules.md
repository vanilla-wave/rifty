...
| `node:` built-ins | ⚠️ | Registry supports `node:` and bare built-ins; each module is a tested subset. `node:constants` exposes the flattened constants available in rifty: supported `fs` constants, rifty's Linux-ABI `os` constants, and `crypto.constants` static data; unsupported keys throw `NotImplementedError('constants.<key>')` instead of returning `undefined`. `node:vm` covers `Script`, `createContext`, `isContext`, `runInThisContext`, `runInContext`, `runInNewContext`, and `compileFunction`; default engine is a real QuickJS realm (cross-realm isolation), without timeout/`displayErrors`/`cachedData`/`contextExtensions` support (those throw loudly). See the node:vm section below. |
