...
REMAINING (acceptance 4 — the item stays open until done): per-option retirement of the wrapper's forced options — `allowedHosts`/`host` (candidate: generic request-Host rewrite at the preview bridge; retirement additionally blocked by `service-worker/preview-blocked-host-hang` — a rejected-Host response is LOST by the chain and the dispatch parks, reproducible even with a localhost Host; diagnose+fix there first), `strictPort`, `base: './'`, `optimizeDeps.noDiscovery` (re-test under wasm esbuild), user-config discovery env (`RIFTY_VITE_CLI_USER_CONFIG`) — then delete `withViteCliArgs`/`withViteCliEnv` + the wrapper file. Residuals riding on the wrapper until then: `RIFTY_VITE_CLI_HMR_OFF` (ADR-0161 Vite 8 hmr-off pin) and the `configureServer` server-handle plugin (needs a home if the wrapper dies — vite still needs editor-write invalidation, the VFS has no watcher events). wss-external-stays-native is unit-pinned (browser-client-script test), no e2e egress case.
