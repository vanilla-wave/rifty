...
DELIVERED (acceptance 1–3 + 5): generic marker-guarded (`data-rifty-ws-bridge`) injection inside `serveCrossRealmPreview` (buffered html reply, content-length re-stamped); client `previewPortFromPath` remap (loopback/page-hostname → guest-port channel, original URL kept in the `open` frame, non-loopback stays native); `HttpServer` upgrade accepts foreign-URL-port opens (channel = port intent); wrapper HMR half DELETED (`server.hmr` rewrite, `pluginSource` client injection, `RIFTY_VITE_CLI_HMR`/`RIFTY_VITE_CLI_PORT` env, `viteHmrClientScript`/`createHmrBridgeVitePlugin`, dev-server-boot bespoke hmr wiring) — stock vite HMR proven end-to-end (integration `vite-hmr-channel` stock case + `manual-vite-install` e2e); socket-lab `browser-preview-websocket` = supported (round-trip probe) + new `preview-websocket-bridge.spec.ts` (bare `ws` echo via explicit `ws://localhost:<port>` in the preview frame). Editor-write invalidation survives via a minimal `configureServer`-only wrapper plugin (`__riftyActiveViteServer`).

REMAINING (acceptance 4 — the item stays open until done): per-option retirement of the wrapper's forced options — `allowedHosts`/`host` (candidate: generic request-Host rewrite at the preview bridge), `strictPort`, `base: './'`, `optimizeDeps.noDiscovery` (re-test under wasm esbuild), user-config discovery env (`RIFTY_VITE_CLI_USER_CONFIG`) — then delete `withViteCliArgs`/`withViteCliEnv` + the wrapper file. Residuals riding on the wrapper until then: `RIFTY_VITE_CLI_HMR_OFF` (ADR-0161 Vite 8 hmr-off pin) and the `configureServer` server-handle plugin (needs a home if the wrapper dies — vite still needs editor-write invalidation, the VFS has no watcher events). Parity case 2's close-event extension to the preview path rides the socket-lab probe; wss-external-stays-native is unit-pinned (browser-client-script test), no e2e egress case.
