...
REMAINING (acceptance 4 — the item stays open until done): delete `withViteCliArgs`/`withViteCliEnv` + the wrapper file (user-config discovery env `RIFTY_VITE_CLI_USER_CONFIG` rides the wrapper). Residuals riding on the wrapper until then: `RIFTY_VITE_CLI_HMR_OFF` (ADR-0161 Vite 8 hmr-off pin), `RIFTY_VITE_CLI_NO_DEP_DISCOVERY` (ADR-0192 template gate), and the `configureServer` server-handle plugin (needs a home if the wrapper dies — vite still needs editor-write invalidation, the VFS has no watcher events; PR #115 recursive-watch may make the STOCK watcher honest — re-check after it lands). wss-external-stays-native is unit-pinned (browser-client-script test), no e2e egress case.
