...
created: 2026-06-08
why: Effect's Readable.fromWeb(body).pipe(res) path unsupported until Readable.fromWeb lands in @riftydev/io (no owner); broader M11 ask is full node↔WHATWG stream interop (toWeb/fromWeb both ways + node:stream/web)
sources: [M11, docs/research/open-webcontainers-alternative-2026-06.md, Q-2026-05-30-109, Q-2026-05-30-103, feature-05-effect-http-bridge T4]
---
## Context
...
Broader M11 scope (one home for the stream/web surface): today only one-way WHATWG→Node conversion exists (internal `pipeBodyStream`). Full interop adds `Readable.toWeb`/`fromWeb`, `Writable.toWeb`, and a `node:stream/web` export — pull each behind a verified consumer (e.g. fetch-/undici-shaped code), keeping the deferral rationale above.
