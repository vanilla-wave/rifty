...
Broader stream/web scope (one home for the surface): today only one-way WHATWG→Node conversion exists (internal `pipeBodyStream`). Full interop adds `Readable.toWeb`/`fromWeb`, `Writable.toWeb`, and a `node:stream/web` export — pull each behind a verified consumer (e.g. fetch-/undici-shaped code), keeping the deferral rationale above.
