...
How rifty is **consumed**, separate from the runtime milestones in docs/ROADMAP.md. This area = umbrella SDK, deep project sessions, framework bindings + component kit, IDE starter template. This is a **pull backlog** (no dates, only `depends-on` edges); several items are required for the full Consumer Ready story, but are deliberately outside the current branch cutline.
...
- **Deep workbench behind your UI**: +C — files/packages/run/terminal/preview through project-session handles.
