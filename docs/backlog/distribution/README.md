...
- **DD-3 — `@riftydev/workbench` (headless UI controllers)** — framework-free, browser-only, host-configured, one active session per page; playground dogfoods the package and Solid stays in the binding layer. **Ratified: ADR-0224.**
...
| `workbench-controllers.md` | ready | embeddable-dev-loop | implementation + local package proof complete; lockstep npm publish + clean public install remain |
...
```
A (publish, landed) ── B (umbrella/SDK, landed) ── C (workbench, ADR-0224) ── react-bindings ── embed-host-vite-example   [= epics/embeddable-dev-loop]
...
- **Headless workbench behind your UI**: +C. Implemented locally under ADR-0224; lockstep publish pending; Vite verified, other hosts unverified.
- **Embed with your UI, ready components**: +react-bindings — `epics/embeddable-dev-loop`, the current distribution epic.
...
These pull the remaining consumption tracks above (create-rifty-template, exec/preview, snapshot/fork, and bindings over the ADR-0224 workbench) as their downstream conversion path, but do not block on them.
