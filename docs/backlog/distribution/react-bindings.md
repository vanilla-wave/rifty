...
Thin React binding over ADR-0263/0273's `@riftydev/workbench` project-session handles (carved out of the old EPIC D kit; vue/`<RiftyIDE/>`/default theme stay in the residual `distribution/framework-bindings-kit`). Consumer owns layout and styling; components auto-wire through provider context without exposing Workbench internals.
