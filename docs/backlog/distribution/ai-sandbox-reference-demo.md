...
1. **Live preview is NOT public.** The public `Sandbox` surface is `runtime.eval` + events + `fs` (`packages/runtime-js/src/host.ts`). An eval'd server's `listen()` registers its port only in the runtime-js worker realm's net registry; nothing installs a cross-realm preview-port handler there — that wiring lives in workbench (`packages/workbench/src/workers/node-program-lifecycle.ts`, `packages/workbench/src/glue/preview-bridge-wiring.ts`) and is not exported by `Sandbox`. The booted-server→SW-preview path therefore needs the parked, IRREVERSIBLE `distribution/public-api-ai-agent-exec-preview` (its draft notes "preview wiring stays SW/host-route specific"). Hence `blocked_by`.
