...
created: 2026-07-10
why: the lowest-effort distribution tier (StackBlitz-embed-style) — a docs/tutorial author drops an <iframe> + gets a live Node example; recorded per user call 2026-07-10, deliberately NOT in the embeddable-dev-loop epic
---

## Context

`<iframe src="https://rifty.dev/embed?...">` + postMessage control. Different persona than the SaaS own-UI epic: docs/course/tutorial authors who won't install packages.
...
- (b) no-COI degraded mode — against fidelity as a silent degrade; if ever, must be a loud capability gate (which features throw), not a quiet subset.
