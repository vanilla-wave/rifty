...
Product tree: `b673665c3`. Final I8 proof: `b7fd5802f`. Final export proof:
`c6d8b480b`; re-chart: `97d47aa3e`. Node 24.16.0, Playwright 1.60.0,
...
pnpm pr:check
# final export tree: 24/24 PASS; test:run 224.4s; parity 75.7s
```

Remote `no-coi-chromium` passed 39/39 at `b7fd5802f`. Its ordinary review:
3/3 coverage pass, no findings, product delta 0. Final export ordinary review
at `c6d8b480b`: 2/2 coverage pass, no findings, product/test delta 0. The
dev-HMR Final+GREEN at `b673665c3` found 0 blockers and 0 missing coverage.
...
clean-report exactness and unowned registry callback arity. Old ledger line
158 called them untraced; line 161 corrects that shorthand — dispose silence
is traced, but its carrier was graded weak rather than missing.

## Exhaustive ledger export

Snapshot before deletion: 176 lines, bullet entries 3–176. Ranges below are
...
|---|---|---|
| 3–9 | carrier | scope/refit decisions, one-off disposition below, build/HMR/degradation/SW records |
...
| 160–171 | carrier | first close audit, CI proof evidence, process finding, ordinary PASS/re-chart |
| 172–176 | carrier | second close audit, export evidence, declined rows, ordinary PASS/re-chart |

Ledger line 7 carrier, explicitly: `util-types.ts:27,31` are TypeScript
predicate positions erased at runtime; adjacent runtime checks compare brand
strings. No bare `SharedArrayBuffer` global read or product gap exists there.
`no-coi-closure-export-proof-evidence.md` retains the exact audit and result.
...
|---|---|
| 20 | carrier: capability report plus loud `execSync` no-COI test |
| 21–24 | carrier: ADR-0375 plus Vite 8/threaded-WASM named-boundary test |
| 25–28, 44–47 | carrier: SW probe plus declined SW-delivered-COI row |
| 29 `ring-less spawn` | dropped: outside tier `works`; no code/spike; `kernel/process-equals-web-worker` retains real isolation scope |
| 30 `async remote-fs` | dropped: outside goal; no trigger, code or spike |
| 30 `snapshot children` | dropped: outside goal; no trigger, code or spike |
| 30–31 `sync-XHR-to-SW` | dropped: zero code/spike; goal chose one in-realm Worker |
| 32 | carrier: `distribution/iframe-embed` |
| 33–38 | carrier: ADR-0377 plus declined heartbeat/journal row |
| 39–42 | carrier: ADR-0375 plus declined Vite-identity-policy row |
| 43 | carrier: ADR-0165 |

Declined concepts also contains both ADR-0378 alternatives: realm-wide
async-source census, and a fresh Worker before every initial resident start.
