...
sources: [ADR-0224, ADR-0003 (D-002)]
code: [packages/workbench/src, apps/playground/src]
---

## Context

Implementation and local acceptance are complete on the workbench branch; the item remains open solely until the next lockstep 0.x release is published and a clean public-registry install passes. The embed core now lives in `@riftydev/workbench` (layer: above sdk/runtime packages, below any UI binding), and the playground consumes it. Presentation prefs (layout-store, splitter-size, fonts, terminal-theme, launcher-prefs, preset-deep-link) stay in the app. Boundary vs the parked SDK API items (`public-api-ai-agent-*`): those are sandbox-level headless APIs (exec/snapshot for agents); workbench is the UI-session layer above the sdk.
...
- ADR-0224 ratifies DD-3: package name `@riftydev/workbench`, layer position, framework-free API shape, browser-only construction, host-injected assets/registry, Vite-first verification, and the single-instance rule.
...
| `provenance-lie` × preview LIVE state | LIVE only after version-matched PONG from the controlling rifty SW plus a routed HTTP proof; registration/control/scope failure → loud preview error state, never optimistic LIVE |
