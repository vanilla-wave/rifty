...
- The actual rifty.dev VM deploy of the eddy image — confirm-first/outward (operator infra); the recipe + compose land here, the `yc ... --docker-compose-file` run does not. (The **v1.1** image IS live on rifty.dev since 2026-07-01, CDN tier included — `docs/public/hosting-eddy.md`; what remains outward is the **v1.2** REDEPLOY of this batch's image, on which `perf/eddy-upstream-registry-ab` is blocked.)
