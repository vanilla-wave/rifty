...
## Options / Next
The fallback exists because non-isolated dev/Node lacks the SAB+worker-url prerequisite. Next: decide whether the in-realm path stays as an honest degraded mode (documented in compat) or is removed once COI is guaranteed; if kept, ensure it never silently masquerades as isolated. Tied to runtime-js/parallel-worker-threads (same gate) and to the kernel server-process work.
## Reversibility
