...
## Options / Next
Ambiguous-verify item: (a) tick the two M8 boxes against the ADR-0014
unified-backend reality once confirmed; (b) add a dedicated test: write a file
via node:fs, run a real WASI binary (reuse ADR-0316's exact package-sourced
preview1 guest/fixture, or a tiny cat-style guest) that reads it *through a
preopen* (e.g. `/workspace`), assert the bytes round-trip both directions. This
generalizes the single-source-of-truth claim past stdin transform proof.
Confirm whether ADR-0049's esbuild test already covers read-through-preopen or
only stdin/cwd-open.
## Reversibility
