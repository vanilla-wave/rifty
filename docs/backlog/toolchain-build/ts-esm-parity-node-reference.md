...
## Reversibility
REVERSIBLE. No package public API — `run-in-node.ts`/`types.ts` are a `tools/` harness, no `src/index.ts` change. Revert <20 lines, 2 files (restore the `NODE_MAJOR < 23` branch + TSDoc). No new dep (tsx pre-existing). Review by end of M12.
