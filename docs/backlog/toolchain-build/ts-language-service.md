...
- `playground/ts-language-service-sandbox-preset` — UX: a one-click `.ts` preset that exercises every shipped feature in-place (the default project is JS, where the LS is honestly empty, so there's no discoverable "try TypeScript" surface today).
