...
Replace remote. Replace uses CAS against the exact displayed conflict version
or displayed missing entry (`null`); another remote change produces a new
conflict, never an unconditional overwrite or new public API. Closing/switching
must retain the existing dirty-close guard. Never infer overwrite from another
edit or Save, and never discard either byte sequence before the user chooses an
outcome. RED cases must cover same-length remote replacement, remote deletion,
repeated debounce, and project teardown.
