...
- Coordinate but do not merge with `owner-vfs-mutation-terminal-certainty`: these protocols have different authorities and result shapes. Decide the shared-vs-separate retention scaffold against `playground/correlated-broadcast-bridge-helper` (five hand-rolled correlation engines already exist) before implementation. Persistence health after a terminal durability error stays in `workbench-persistence-health-propagation`.
