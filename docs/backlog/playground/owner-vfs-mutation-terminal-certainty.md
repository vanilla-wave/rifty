...
- Design the retention/settlement scaffold together with `session-tool-mutation-terminal-certainty` against `playground/correlated-broadcast-bridge-helper` (five hand-rolled correlation engines already exist): decide shared-vs-separate before implementation; do not mint another bespoke copy.
