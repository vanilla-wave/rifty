...
- Candidate acceptance must prove an untouched real Vite project stays clean and reopens offline with zero reacquisition, while ordinary dependency mutations still revoke and every real Vite entry path/config form remains Node-parity faithful. A fake config loader, a path whitelist, or an install-count metric alone cannot close it (guard retained from the rejected contract).
