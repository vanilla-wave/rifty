...
- **Observed while certifying PR #145, not expanded there:** the GREEN Chromium-light first-run/reset sequence can emit an unhandled `BroadcastChannel.postMessage` after its channel has closed. Refinement must minimise the exact sender/teardown edge and give late lifecycle publications an owned cancellation or terminal outcome; swallowing the exception would preserve the race. Relevant paths: `orchestration/workspace-lifecycle.ts`, `glue/project-index-port.ts`, `tests/e2e/m0-boot.spec.ts`.
