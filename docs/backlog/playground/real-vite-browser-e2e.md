...
## Options / Next
Next: a Playwright spec driving the full path in a COI browser — load → toggle Real Vite → worker boots Vite → iframe-preview renders via SW `/preview/<port>/` routing → HMR update on edit, incl. ADR-0048 streaming preview for large bodies (vendor-prebundle, source maps). `m10-hmr.spec.ts` exists but is gated behind `RIFTY_E2E_HMR=1` (skipped by default, ~20 s Vite install per run) and depends on the SW iframe-nav bug (see service-worker preview-subframe-nav-owner-routing). Update the `docs/backlog/playground/` status once green.
## Reversibility
Reversible — adds a test + CI gate, no production-API change. Blocked-ish: depends on the SW iframe-nav owner-routing fix landing for a non-skipped run; otherwise stays opt-in.
