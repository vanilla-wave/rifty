...
- Tripwire automation beyond `check:contract-drift`: scope-outside-ready-items (diff→item mapping), new-mechanism detector (pending-map/opId shape inventory), per-item diff-mass vs Budget estimate, review-round counter. Semi-manual (review axis) until each detector is honest — no fake precision.
