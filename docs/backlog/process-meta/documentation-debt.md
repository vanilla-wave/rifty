...
- **(A) ADR-0090 mislabel + stale acceptance block.** File H1 reads `# ADR 0083` (collides with the active statSyncOrNull ADR), and the historical acceptance checkboxes remain unchecked even though the VFS primitives shipped and the playground `renamePath` migration now uses `renameSync`. Tracked via `docs/adr/README.md` correction provenance; do not rewrite the active ADR body.
...
Excluded from this ledger (NOT doc-only, kept as their own items): the esbuild substitution-strategy fork (the pre-0316 item `npm-client/esbuild-substitution-strategy-reconciliation` was removed — ADR-0311/0316 settled mechanism ownership; the surviving N=1 asset-chain question is `npm-client/esbuild-wasm-twin-recut`); `service-worker/anonymous-embedded-heuristic-warn-mismatch` (likely a code fix — add the warn); `playground/dev-mode-repl-retirement-dead-code-unrecorded` (dead-code removal); `service-worker/page-proxy-retirement-untracked` (code-retirement follow-up); the compat-matrix coverage (`process-meta/compat-matrix-coverage-debt` — generator code + parity cases); the test-coverage debt (`process-meta/test-coverage-debt`); the `refs:check`/`madge` tooling guards; and the `kernel/resolve-getipcmode-dead-public-surface` code item.
