...
Excluded from this ledger (NOT doc-only, kept as their own items): `npm-client/esbuild-alias-override-retirement` (design/impl fork — dead ~20MB alias download; subsumed the former `esbuild-substitution-strategy-reconciliation`); `service-worker/anonymous-embedded-heuristic-warn-mismatch` (likely a code fix — add the warn); `playground/dev-mode-repl-retirement-dead-code-unrecorded` (dead-code removal); `service-worker/page-proxy-retirement-untracked` (code-retirement follow-up); the compat-matrix coverage (`process-meta/compat-matrix-coverage-debt` — generator code + parity cases); the test-coverage debt (`process-meta/test-coverage-debt`); the `refs:check`/`madge` tooling guards; and the `kernel/resolve-getipcmode-dead-public-surface` code item.
