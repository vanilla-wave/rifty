...
- **(E) Stale "in-realm execBin" prose (pty-server).** `packages/workbench/src/workers/pty-server.ts` still describes in-realm bin exec; P6a wires `createOwnerChildBinExecutor` (a supervised child worker). Correct the comments.
...
- **(E) Dead `RIFTY_OWNER_MODE` env.** `packages/workbench/src/glue/realVite.ts` sets `env RIFTY_OWNER_MODE:'shell'` but it is read nowhere; `packages/workbench/src/workers/real-vite-bootstrap.ts` implies a branch that no longer exists. Drop the set + the comment (one-line dead-code removal alongside the doc fix).
