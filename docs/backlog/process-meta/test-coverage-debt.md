...
- **Responsiveness under read load — owner-thread isolation half still untested.** `owner-responsive-under-load.spec.ts` now asserts the PAGE main thread stays responsive (command palette opens promptly) during the co-resident dev-server boot — the canonical read-heavy load. Still uncovered: the SHELL/owner-thread half — a concurrent shell session not being starved while the dev server transforms co-resident in the SAME owner thread. That needs the dev server moved to a supervised child (its own work) or a shipped CPU-hog bin to drive a deterministic owner-thread-busy probe.
