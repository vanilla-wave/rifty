...
created: 2026-07-26
why: a two-line `// TODO(backlog: …)` relink in .ts counts as production source beside a ready-flip, forcing an illegal second PR (`AGENTS.md` §PR — unit of delivery; real-node-server-dev-loop session, PR #152)
...
absorbed/renamed item) is contract plumbing, not source — yet it moves pickup
and forces a second PR. Candidate: classify a file as source only if some changed hunk
