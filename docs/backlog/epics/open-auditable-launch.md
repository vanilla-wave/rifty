...
- 2026-09-02 (user, via `playground/react-vite-starter` go/no-go budget (b)): the deep-link tile is now the React 19 issue-tracker starter; "<5s cold start" retired — that tile measures 9.8 s on the standard transport, and the eddy figure stays unmeasured until `distribution/eddy-live-esbuild-closure-decline` closes. The launch quotes `perf/benchmarks.json` as re-measured on the tile the link boots, never a stale tile's figure.
