...
items: [playground/launch-deeplink-real-npm, perf/cold-start-and-install-benchmark, distribution/readme-open-auditable-rewrite, distribution/publish-git-and-ts-language-service]
---
...
- `perf/cold-start-and-install-benchmark` — the ONE measured, defensible number + GIF source; quoting an unmeasured figure is forbidden by the Fidelity rule. Blocking.
- `distribution/readme-open-auditable-rewrite` — the star-conversion landing reframed to the wedge (blocked on the GIF + benchmark above).
