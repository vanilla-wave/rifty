...
items: [runtime-js/stream-web-module-and-readable-toweb, runtime-js/stream-statics-predicates-and-defaults, runtime-js/stream-writable-duplex-web-bridge, runtime-js/stream-async-iterator-helpers, runtime-js/stream-cork-uncork-writev-batching, runtime-js/stream-compose-wrap-duplex-from]
---
...
- `runtime-js/stream-web-module-and-readable-toweb` (**ready**) — register
  `node:stream/web`; `Readable.toWeb`. Highest library reach.
- `runtime-js/stream-statics-predicates-and-defaults` (**ready**) — `isReadable`/
  `isWritable`/`isErrored`/`isDisturbed`; `get`/`setDefaultHighWaterMark`;
  `stream.addAbortSignal` (export the half-built one). All S-effort statics.
