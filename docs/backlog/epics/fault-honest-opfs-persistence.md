...
items: [vfs/opfs-persist-hang-watchdog, vfs/iso-git-ref-torn-write-rows, vfs/persist-ledger-fault-rows-completion, playground/reload-crash-consistency-fault-e2e]
---
...
- `vfs/opfs-persist-hang-watchdog` (ready) — a never-settling persist op becomes a bounded, ledger-recorded failure; `flush()` always answers.
