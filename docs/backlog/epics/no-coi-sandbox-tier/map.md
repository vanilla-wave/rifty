...
1. `runtime-js/sandbox-toolchain-memory-descriptor-evaluation` —
   **memory-descriptor** — package-generic native descriptor count/order and
   stateful shared-getter safety at the existing runtime-js realm seam, with
   REPL/CJS/ESM/installed-bin siblings. No public API/protocol/state.
2. `distribution/no-coi-sandbox-build-loop` — **build-loop** — sandbox
   ...
   (I1, I2, I3, I7 rest, I8, I9). Blocked by memory-descriptor.
