...
- ~~`perf/install-transport-tuning`~~ — CLOSED 2026-07-01: preconnect SHIPPED (ADR-0195, boot preconnects registry + resolver origins), fetch-semaphore raise DROPPED (measured inert — one coalesced h2 connection per origin), the h3 lever folded into `perf/eddy-http3-cold-validation`.
