...
blocked_by: [npm-client/shadow-materialized-bin-authority]
...
`npm-client/shadow-materialized-bin-authority` lands and remains deliberately
unmapped from the epic Items/Budget until its own pre-pickup Contract+RED
window.
...
successor supplies one settled linker/commit boundary; this item adds no codec,
catalog, resolver, cache, lock, FIFO, or public recipe owner.
