---
area: npm-client
status: draft
title: Shadow recipe v2 acquisition and embedded-source authority
created: 2026-07-28
why: the blocked recipe-v2 predecessor proved the current LightningCSS registry twin is neither verified as an exact acquisition nor consumed as one embedded bundle
user_story: As a browser-IDE user installing a registry-backed substitution, I want the exact reviewed registry source and its embedded dependency consumed as one package, but today rifty fetches the bundled child separately
epic: honest-shadow-substitutions
sources: [ADR-0335, docs/backlog/npm-client/reference/lightningcss-wasm-1.32.0-packument.md]
code:
  - packages/npm-client/src/installer.ts
  - packages/npm-client/src/internal/shadow/planner.ts
  - packages/npm-client/src/linker.ts
---

## Context

This is the first dependency-ordered acquisition/embedded-source successor to blocked predecessor
`npm-client/shadow-recipe-v2-authority` / PR #212. The materialized-bin
successor landed through PR #237 on 2026-08-02 and is present at fresh baseline
`main@4a2beb233cc2127ef531b0eba2584797234865f1`. This successor is now epic
Item 19 with source Budget `300–800`; it remains draft until its fresh isolated
Contract+RED checkpoint passes review.

Checkpoint `812cd8b0e5c653674bae949d67f0ac21db90748f` was blocked because it
combined this source unit with protocol-v2 replay and acceptance consumers,
used generated archive evidence instead of the real tarball at installer
ingress, and prescribed an unratified error-cause shape. This in-place re-cut
keeps the first source unit only. Draft child
`npm-client/shadow-recipe-v2-protocol-replay-authority` owns protocol-v2 trace,
literal v2 replay, v2 Eddy completeness, Workbench FIFO assertions, and
Chromium v2 lock proof after this item lands.

The existing installer/planner/linker seam owns source projection,
embedded-bundle evidence, alias publication, and fresh lock facts. The
materialized-bin successor supplies the settled linker/commit boundary; this
item adds no codec, catalog, resolver, cache, lock, FIFO, trace protocol, or
public recipe owner.

## Reference contract

- `lightningcss-wasm@1.32.0` registry identity, integrity, complete dependency
  maps, bundled `napi-wasm` membership, and all four embedded
  `napi-wasm@1.1.3` members are pinned from the real npm tarball. Contract proof
  feeds those integrity-verified tarball bytes through the real fresh installer;
  generated miniature archives are mutation inputs only.
- The completed schema-2 data authority owns recipe codec, catalog identity,
  admission mode/feature, and schema-1 identity detection. This successor
  consumes those facts without adding a second data or trace owner.

## Acceptance

- Consume the completed clone-safe schema-2 data authority and preserve its
  `semver-admits`/`exact-only` result and named feature through every execution
  path; this item does not add a second codec or admission owner.
- Registry acquisition verifies the current exact LightningCSS projection
  before tarball work: required `napi-wasm@^1.0.1`, bundled `napi-wasm`, and
  empty optional and peer maps. Removing or changing any required member, or
  drifting any complete map, loud-fails the recipe's named feature before
  tarball, cache, VFS, tree, report, or lock work.
- The bundled member stays embedded under
  `lightningcss-wasm/node_modules/napi-wasm`; its manifest names
  `napi-wasm@1.1.3`, satisfies the recorded range, and all four real embedded
  members survive extraction. It never enters ordinary registry traversal,
  cache, or a root install. Root and nested fresh lock facts record exact
  acquisition dependencies and bundle membership plus the child's exact
  version and `inBundle: true`.
- Matching current-protocol replay and the existing generic Eddy ingress reuse
  the same integrity-verified parent bytes, retain the embedded member, and
  perform no standalone child registry or cache operation. Protocol-v2
  provenance and corruption authority remain downstream.
- Acquired registry-twin bins remain absent from disk and acquisition lock
  facts. Registry alias materialization uses the materialized-bin successor's
  phase and commit authority; its exact files and bin map settle before
  substitution reporting and lock publication.
- Preserve the completed v2 catalog/install/snapshot identity migration and
  every drift gate. Add concise npm-client and shadow-registry CHANGELOG entries.

## Parity cases

1. Root and nested LightningCSS installs accept their current semver requests,
   verify exact registry maps, and feed the integrity-verified real
   `lightningcss-wasm@1.32.0` tarball through the fresh installer.
2. The installed parent retains all four real embedded `napi-wasm@1.1.3`
   members; no standalone registry, cache, root-tree, or lock source exists for
   that child.
3. Fresh root and nested locks match npm's parent dependency/bundle facts and
   embedded child `version`/`inBundle` facts.
4. Removing, adding, or changing each complete acquisition map or bundle member
   rejects with `lightningcss.acquisition` before tarball/cache/VFS/report/lock
   effects. A missing embedded manifest or wrong child name/version rejects
   after the bounded parent acquisition but before VFS/report/lock effects;
   generated archives are used only for these mutation cases.
5. Matching current-protocol replay and generic Eddy ingestion preserve the
   same parent/embedded bytes without a child source; existing schema-2
   admission, acquired-bin suppression, alias materialization, and ordinary
   registry fault behavior remain green.

## Fault matrix

| Fault class | Required outcome | Proof |
|---|---|---|
| corrupt-input / provenance-lie | complete registry maps strict-compare; the SRI-verified real tarball and embedded manifest/member set must agree with the recipe | independent map mutations, real-tarball root/nested carrier, and generated missing/name/version embedded-manifest mutations |
| observable-order | admission precedes registry; projection precedes tarball/cache/VFS; embedded-manifest validation precedes link/report/lock | fresh-install operation ledgers |
| unbounded-read | registry headers/bodies remain progress-bounded, runaway bodies capped, discarded responses cancelled | inherited `RegistryClient` fault suite (8/8) |
| torn-state | parked alias abort stops later writes, emits no report/result/lock, and exact retry reconciles | root/nested alias write ledger |
| quota-perm-fail | alias `ENOSPC`/`EACCES` emits no report/result/lock and exact retry reconciles | root/nested alias fault table |
| sibling-drift | direct/transitive fresh ingress shares generic recipe and projection owners | data-authority matrix plus finite generic-source gate |

## Out of scope

- npm same-command collision settlement; it remains
  `NotImplementedError('npm-client.bin-collision-reify')` + compat ❌ and is
  tracked by `npm-client/npm-11-bin-reify-authority`.
- Matching non-bundled required and retained-optional traversal, omitted
  optional suppression, non-empty peer handoff, and accepted scoped keys;
  `npm-client/shadow-recipe-v2-dependency-projection-execution` owns them.
- Protocol-v2 trace/materialization-bin provenance, literal-v2 offline
  root/nested replay, corruption gates, cached embedded-manifest validation,
  v2 Eddy completeness, Workbench FIFO assertions, and Chromium v2 lock proof;
  `npm-client/shadow-recipe-v2-protocol-replay-authority` owns them.
- Peer traversal/placement/replay, the Sass recipe, a public/custom recipe SPI,
  raw concurrent public `install()`, or any second resolver, cache, lock, FIFO,
  or coordination mechanism.

## Decisions

- `2026-08-02` fresh baseline
  `main@4a2beb233cc2127ef531b0eba2584797234865f1` includes the completed
  materialized-bin successor from PR #237. This in-place acquisition re-cut
  remains epic Item 19 with its original Budget `300–800` and stays draft
  pending a replacement reviewed Contract+RED.
- Standards blocked checkpoint
  `812cd8b0e5c653674bae949d67f0ac21db90748f`; it is lineage, not a readiness
  verdict. The dependency-ordered split appends protocol/replay as epic Item 20
  without rewriting Item 19 or its Budget history.
- Spec blocked replacement checkpoint
  `f5dbb4e021380dbdbbd964e33b434e47c2348618`: its official-tarball positive
  proof was complete, but no negative RED forced embedded manifest identity and
  version/range validation before link effects. The next checkpoint adds that
  generated-tar mutation table; the official archive remains the sole positive
  oracle.
- The acquisition carrier must feed the integrity-verified real pinned npm
  tarball through the fresh installer and observe all four embedded members.
  The committed JSON identity and generated miniature archives cannot replace
  that proof; generated bytes serve mutations only.
- `split-predecessor:
  87098d2dbaeb5665188b98cbc1a5cdd6ba876cf9`; checkpoint lineage:
  `8f3251e89020772f15ff5a13022e7f7310f703d2`,
  `d5ffb3d2de8a27b26a13f541d2e5d16260d4b8d8`,
  `5c450fb9a5cab66a45b24eb8b19a1729c622e5a9`,
  `b7725a3e88278f4f24efb1d8c8d90e80de08de43`,
  `092d931a533ea45fa060367bd9373f78a7f2c684`, and
  `87098d2dbaeb5665188b98cbc1a5cdd6ba876cf9`.
- The installer/planner/linker seam remains one deep source module: exact
  projection, embedded-bundle evidence, alias publication, and fresh lock facts
  stay behind the existing install interface.
- The original acquisition entry retains its complete required map for lock
  fidelity while ordinary traversal filters verified bundled members.
- Protocol v2, replay, Workbench FIFO, and Chromium lock assertions are
  downstream behavior, not prerequisites for validating the acquired source.
