...
code: [packages/npm-client/src/installer-request.ts]
---
...
`*.tgz` values … uppercase `NPM:` aliases read as a package name"); the
same classifier guards `dependencies`, so it is not override-only.
...
(dependency) or, for overrides, a replacement package (ADR-0451 decisions
4/6, `overrides.ts` `parseTarget`: `NPM:vite@8` → package `NPM:vite`
range `8`; `8.0.16.tgz` → package `8.0.16.tgz`) — a registry read of that
name. `@scope/pkg` as an override value is also a rifty-dialect question (`npm-client/overrides-rifty-dialect-vs-npm`); as a dependency
