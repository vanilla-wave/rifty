...
user_story: As a developer re-running `npm install` after a root optionalDependency failed (native-unsupported / lifecycle-reject), I want the lockfile fast path to skip re-resolving the whole tree, but currently `lockfileCovers` returns null forever because the dropped optional is never pinned — full registry re-resolve every install.
