...
```sh
RIFTY_SASS_PROBE_ROOT=./node_modules/.pnpm/sass@1.100.0/node_modules/sass \
node -e "const fs=require('node:fs'); const root=process.env.RIFTY_SASS_PROBE_ROOT; const source=fs.readFileSync(root+'/sass.dart.js','utf8'); console.log(process.version); console.log(require(root+'/package.json').version); console.log(source.includes(\"typeof process !== \\\"undefined\\\" && (process.versions || {}).hasOwnProperty('node')\")); delete process.release; const sass=require(root); try { sass.compileString('@use \"__rifty_missing__\";'); } catch (e) { console.log(String(e.message).split('\\n')[0]); }"
```
...
```sh
RIFTY_SASS_PROBE_ROOT=./node_modules/.pnpm/sass@1.100.0/node_modules/sass \
node -e "const root=process.env.RIFTY_SASS_PROBE_ROOT; delete process.release; delete process.versions.node; const sass=require(root); try { sass.compileString('@use \"__rifty_missing__\";'); } catch (e) { console.log(String(e.message).split('\\n')[0]); }"
```
