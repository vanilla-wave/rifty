...
const classifyValues = [
  ...
  'vite@8.0.16',
  '$vite',
  // Grammar sweep: npm-package-arg reads through node-semver's LOOSE parser,
  // which drops invalid tokens (`1.2.3 foo`) and trims operator whitespace.
  ' * ',
  ' ',
  '>= 8.0.0',
  '>=8.0.0  <8.1.0',
...
const installCases = [
  ...
  ['star', debugTree({ ms: '*' })],
  ['empty', debugTree({ ms: '' })],
  // Only the exact strings '' and '*' keep the edge spec; padded ones are ranges.
  ['star-padded', debugTree({ ms: ' * ' })],
  ['whitespace-only', debugTree({ ms: ' ' })],
