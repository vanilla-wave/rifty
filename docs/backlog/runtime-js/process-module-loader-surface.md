...
user_story: As an author running a real-Node lib or CLI, I want emitWarning/--env-file/SourceMap/data:-import to behave like Node, but today deprecation warnings vanish, --env-file is unparsed, and data: import hard-throws.
...
Per-feature promotable; land each parity-first (failing parity test vs real Node, then implement). Order: emitWarning (S, isolated) → getBuiltinModule (gated on sqlite-detection refactor + pin update, regression test first) → loadEnvFile/--env-file (M, parser pinned + argv seam) → SourceMap/findSourceMap + sourceMaps toggles (M) → data: import (M) → stripTypeScriptTypes (L).
