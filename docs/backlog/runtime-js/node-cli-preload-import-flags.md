...
mutates the eval realm before its source runs. Its program-entry sibling proves
all five valid spellings (`-r <specifier>`, `--require <specifier>`,
`--require=<specifier>`, `--import <specifier>`, and
`--import=<specifier>`) run before the entry and remain exact in
`process.execArgv`:
