...
created: 2026-07-30
why: Node loads `--require`/`-r` and `--import` modules before eval or entry execution, while Rifty rejects each leading flag before allocating a child.
user_story: As a Node CLI author using startup hooks, I want preload modules to run before my command with exact Node identity, but today Rifty reports each supported Node flag as a bad option.
...
Rifty currently classifies `--require`, `--require=…`, `-r`, and `--import`
as unsupported leading options. It writes `node: bad option: <flag>`, exits 9,
and allocates no child. It does not silently ignore a requested preload.
