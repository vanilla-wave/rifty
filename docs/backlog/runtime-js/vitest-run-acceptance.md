...
- 2026-09-25 — observation (`REV-12`, deferred): after a vm-pool start ceiling rifty prints vitest's `close timed out after 10000ms` teardown lines Node does not (evidence §Observation); unclaimed error path, root cause undiagnosed → routed at the post-land re-chart: draft `runtime-js/vitest-pool-start-ceiling-teardown-timeout`; page vm-pools row states it.
