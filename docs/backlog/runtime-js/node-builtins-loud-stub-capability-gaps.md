...
**Absent — not even a loud throw (a raw `TypeError: … is not a function`, WORSE than a NotImplementedError):** `child_process.execFileSync`, `accessSync` (only the callback/promise `access` exist), callback-form `fs.rm` (only `rmSync`/`promises.rm`), `chmod`/`chown`/`lchmod`/`lchown`, `symlink`/`link` (symlink stays governed by ADR-0050's no-symlink model — `readlink` exists), `fs.statfs` / `fs.promises.statfs`, `readv`/`writev`. Per the no-silent-stub rule these should at least throw a named `NotImplementedError` (+ compat ❌); `accessSync` + callback `rm` are cheap + common → implement on demand. (Reachability rose with ADR-0150 — arbitrary CLIs now run in a child and hit these.)

**Named-loud since ADR-0443** (vitest-run-in-browser item 4): `child_process.spawnSync` and `fs.statfsSync` are present members; every call throws `NotImplementedError('child_process.spawnSync')` / `('fs.statfsSync')`. Real `spawnSync` still needs a status/stderr/signal sync-child protocol beyond the stdout-only `execSync` RPC. ADR-0443 §2 keeps members no claimed consumer links or reads at load absent (a named import is a link-time miss). Probe 2026-09-24 (scratch CJS parity case, `tools/node-parity-runner` `runInNode`/`runInRifty` @ `8c8993649`, `typeof` + a call): Node v24.16.0 has every probed member (`spawnSync`, `execFileSync`, `statfsSync`, `statfs`, `promises.statfs`, `accessSync`, `rm`, `chmod`, `chown`, `lchown`, `symlink`, `link`, `readv`, `writev`) as a function and no `statvfs` (not a Node API — dropped from this list); rifty `spawnSync`/`statfsSync` → `NotImplementedError`, the rest `undefined`.

**Silent, not loud** (a Fidelity defect, own item `runtime-js/async-hooks-silent-stubs`): `misc-stubs.ts` `async_hooks.createHook` returns a hook whose callbacks never run and `executionAsyncId()` returns `0`.
