...
- 2026-09-23 — IMPLEMENT: the three members + `rss` live in `builtins/loud-members.ts`; `fs` / `child_process` objects list them, `NodeProcess` owns `memoryUsage` as a class field (every realm's `new NodeProcess`). `fs.ts` stays under its pin by moving the `NodeJS.ErrnoException` global type to `fs-errors.ts` (behavior-preserving, CHANGELOG). The `fs.md` inventory moved to `tools/compat-matrix-generator/fs-matrix.js` for the same reason (`cli.js` pin; generated output unchanged). Sibling sweep of the vitest tree finds no other absent named builtin import (evidence S1).
