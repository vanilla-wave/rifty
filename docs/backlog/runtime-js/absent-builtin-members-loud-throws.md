...
- 2026-09-23 — IMPLEMENT: the three members + `rss` live in `builtins/loud-members.ts`; `fs` / `child_process` objects list them, `NodeProcess` owns `memoryUsage` as a class field (every realm's `new NodeProcess`). `fs.ts` stays under its pin by moving the type-only `StatOptions` to `fs-stats.ts` (see the Final+GREEN r1 line). The `fs.md` inventory moved to `tools/compat-matrix-generator/fs-matrix.js` for the same reason (`cli.js` pin; generated output unchanged). Sibling sweep of the vitest tree finds no other absent named builtin import (evidence S1).
- 2026-09-23 — Final+GREEN r1 blocker (FIX): the first pin fix moved the `NodeJS.ErrnoException` global to `fs-errors.ts`. tsup's d.ts bundle keeps a `declare global` only from an entry module, so the published `builtins/fs` d.ts lost it: 9× TS2503 for a consumer without `@types/node` and with `skipLibCheck: false`. The global is back in `fs.ts` at its BASE place. Carrier `packages/runtime-js/tests/published-dts.test.ts` runs the package's own tsup d.ts build and a standalone type-check over every emitted d.ts. It is RED on the pre-fix sources and green on the BASE and fixed sources (evidence G3).
...
- 2026-09-25 — DEC-2 decision review of ADR-0443 (partial supersession of ADR-0348 §2 link-only-placeholder clause): justified-with-fixes, evidence I6 + O1/O2/O3/O5 + R2. Fixes applied: ADR-0348 note + `docs/adr/README.md` §Corrections row admit on "whose real behavior rifty cannot supply"; ADR-0443 §Context splits the sync-child protocol gap (O4) from browser-source absence; ADR-0443 §2 covers members read at load (absent, not only link-time misses). Closes the owed review above.
