...
blocked_by: []
sources: [docs/adr/runtime-js/0237-readable-owns-read-hook-dispatch-and-demand-latch.md, docs/adr/runtime-js/0239-fromweb-arguments-have-one-staged-validation-owner.md, docs/public/compat/streams.md]
...
- Readable/Writable/Duplex web adapters are cold, ignore hook getters, read Node
  config once, preserve staged validation/acquisition order, treat falsy signal
  as absent, and loud-throw valid unsupported signal before acquisition.
...
4. All fromWeb adapters: pair/getter/acquisition order, post-lock HWM/invalid
   signal errors, cold pull, valid-signal loud gaps.
...
| `false-fallback` × fromWeb signal | Falsy is absent; invalid values preserve Node post-acquisition errors; valid supported values loud-throw before acquisition |
...
Implement ADR-0237 + ADR-0239 through one core source/argument owner; no adapter waiters or second latch.
