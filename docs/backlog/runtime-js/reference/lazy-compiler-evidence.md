...
- Public loader export contract now includes the ADR-0382 preloader; the existing closed-list test gains only this declared API. Original loader-object and node-entry export lists remain exact. Full gate found this expected list change (1 failure,0 timeouts, reproduced in isolation); no product assertion weakened.
