...
sources: [M11, docs/research/open-webcontainers-alternative-2026-06.md, ADR-0010]
...
is correct today, but auth/session/JWT/cookie-signing libraries are a real consumer class for the M11
"runs real-ish projects" goal. Expanding is NOT a thin SubtleCrypto mapping — the SYNC primitives real
code calls can't be backed by async SubtleCrypto on the calling worker, so it realistically needs
pure-JS implementations (as already done for the hashes), with correctness/timing risk. Parked:
record the deferral + its gate; don't build speculatively.
