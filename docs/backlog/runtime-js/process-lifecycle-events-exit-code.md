...
and this unit absorbs it — the zero-ref drain race only; an already-queued
Node callback that runs before Chromium's `unhandledrejection` task stays with
that draft (fork 2026-09-24). Carrier: ADR-0445.
...
7. `docs/public/compat/process.md`: ✅ rows for handler dispatch, the `'exit'` event, `exit()`/`exitCode` and the late-rejection drain (replacing BASE's drain-race ⚠️ row); ❌/⚠️ rows for each Out-of-scope gap below; `packages/runtime-js/CHANGELOG.md` and `packages/workbench/CHANGELOG.md` lines. → ADR-0445
...
10. With no listener, a throw (timer, `nextTick`, entry) or a rejection prints the error on stderr, emits `'exit'` 1 with `exitCode` 1, and exits 1 — never 0, also when a later task calls `exit(0)`; for a rejection that later task is one that runs after Chromium has delivered the rejection (its `unhandledrejection` task) (cases `fatal-timer`, `fatal-rejection`, `fatal-nexttick`, `fatal-entry`, `eval-fatal-timer`, `fatal-rejection-then-exit`, `fork-fatal-rejection-then-exit`, `worker-thread-fatal-rejection-then-exit`, `exec-sync-fatal-rejection-then-exit`; o14, o15, o20, o34, evidence §F). An already-queued Node callback that runs before that delivery: Out of scope. → I3, ADR-0445
...
| `observable-order` × a no-listener rejection Chromium has delivered vs a later task's `exit(0)` | stderr and the status-1 exit request run at the trap (Chromium's `unhandledrejection` task); the later `exit()` re-emits nothing and requests nothing; status 1 on every kernel child. An `exit()` already requested — in the fatal `'exit'`, or in the turn that made the rejection — keeps its status, no stderr (Node's outcome, evidence §F); one requested by an already-queued Node callback before the delivery: Out of scope (evidence §F2) | `process-exit-lifecycle.test.ts` cases 3–5; browser-unit `fatal-rejection-then-exit`, `fork-fatal-rejection-then-exit`, `worker-thread-fatal-rejection-then-exit`, `exec-sync-fatal-rejection-then-exit` | → I3, ADR-0445 |
...
- Delivery order of `unhandledRejection` against any already-queued Node callback that runs before Chromium's `unhandledrejection` task — a same-turn `setTimeout(0)` or `setImmediate`, timers due at the same time, an fs callback, a `fs.promises` settlement (Node handles the rejection before the next callback: o18, evidence §F2 `runNextTicks`): not claimed; Chromium queues the rejection notification as its own task, so that callback runs first — with a listener only the row order differs; with no listener it can print before the fatal, and its `exit(0)` exits 0 with no stderr where Node prints the error and exits 1 (evidence §F, §F2). Compat ⚠️; the fence route and its cost: backlog `docs/backlog/runtime-js/late-unhandled-rejection-drain.md`.
...
- 2026-09-23 — scope: absorbs `runtime-js/late-unhandled-rejection-drain` (evidence §B: `fatal-rejection`, `rejection-only-handler`, `eval-rejection-handler` exit 0 with no output); ~~the draft is deleted when this unit lands~~ — corrected by the 2026-09-24 re-cut: draft kept, owns the already-queued-callback case.
...
- fork: 2026-09-24 — user: "B: сузить + backlog (Recommended)" — Parity 10's "never 0" for a no-listener rejection holds against tasks that run after Chromium has delivered it; an already-queued Node callback (`setImmediate`, same-due timers, fs callbacks, `fs.promises` settlement) that runs first is Out of scope, compat ⚠️, and its fence goes to backlog `runtime-js/late-unhandled-rejection-drain`.
- re-cut: 2026-09-24 — fork: user answer B (line above) — Parity 10 + fault row narrowed to rejections already delivered by Chromium; already-queued Node callbacks → Out of scope + compat ⚠️ + `runtime-js/late-unhandled-rejection-drain` (draft kept, owns that case; not deleted at land); Acceptance 7 wording names BASE's drain-race row — trace: I3
- 2026-09-25 — DEC-2 decision review of ADR-0445 (partial supersession of ADR-0152 §1/§3, correction of ADR-0157 §1 `exit()`): justified-with-fixes. Fixes applied: ADR-0445 header names each overturned clause with one verb; dated notes in ADR-0152 §1/§3 and ADR-0157 §1; README §Corrections rows dated, 0152 §3 row scoped to rejections with the `exitCode ?? 1` status. Closes the owed pass above.
