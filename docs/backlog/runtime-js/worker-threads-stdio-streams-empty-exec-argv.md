...
5. Every token outside the three families, `fork` usage errors, Worker `stdin: true` and the same-realm cases throw the named `NotImplementedError` of ADR-0449 §1/§2/§6 before any thread id, hold, child or spawn (`packages/runtime-js/src/builtins/startup-options-ceiling.test.ts`). → ADR-0449
...
7. `docs/public/compat/modules.md` (and `process.md` where it lists child and Worker options) states ✅ Worker `stdout`/`stderr` and `fork`/`Worker` `-r`/`-C`/`--experimental-import-meta-resolve` with Node's inheritance; ❌ every other startup flag, Worker `stdin: true` and the same-realm cases (named throws); ⚠️ unclaimed stdout-vs-`'message'` order, the read-stream hold of an `unref()`'d Worker, and a failing Worker preload's `'error'`. → ADR-0449
...
11. browser-unit `eval-parent-fork`: `node -e "require('node:child_process').fork('./fprint.cjs')"` drops the eval pair. Node row: `SX|eval child []`. → ADR-0449
...
| `torn-state` × an invalid or unsupported startup option on `Worker`/`fork` | throws before a thread id, hold, child or spawn exists; nothing half-started | Parity 4; `startup-options-ceiling.test.ts` (spawn spies) | → ADR-0449 |
...
- `fork` usage errors (an operand missing or starting with `-`, an empty `=` value): Node's child exits 9; rifty throws `NotImplementedError('child_process.fork.execArgv')`; compat ❌.
