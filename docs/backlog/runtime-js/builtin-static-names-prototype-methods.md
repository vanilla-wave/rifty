...
- 2026-09-23 — IMPLEMENT: `this`-free `cwd`/`chdir`/`hrtime`/`uptime` are module functions installed as own fields (as `nextTick` already was), `hrtime.bigint` rides the function; `exit`/`kill` are instance-bound arrow fields; the stdout/stderr writer block moved unchanged to `builtins/process-stdio-writer.ts` (process.ts 1224 → 1133 lines).
