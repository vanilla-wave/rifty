...
blocked_by: [runtime-js/cjs-module-record-metadata]
...
mutates `ModuleRegistry`. `runtime-js/cjs-module-record-metadata` owns record
creation, fields, parent/children lifecycle, loaded transitions, and failed-load
unlink. This item begins after that lifecycle exists and owns only one public
cache view plus Node-compatible delete/reload behavior. An already cached ESM
