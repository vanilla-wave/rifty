...
| peer death (fault-classes §Boundary: dedicated Worker) × a kernel peer closing, or the DOM Worker boundary refusing the spawn; a peer death or failed start (refused spawn, `data:` URL gap, same-realm load failure) whose `'error'` has no listener | `'error'` then `'exit'` 1, and every hold the Worker took is released; the parent never hangs. An unlistened `'error'` is the parent's uncaught exception (never held back by the kernel's teardown); `'exit'` 1 follows unless it ended the parent. Node races that `'exit'` against the error (its place, and whether it precedes a fatal uncaught exception, vary with loop timing); rifty takes an unloaded Node run's order: after the `'error'` handling's microtasks when listened, between the uncaught exception and its microtasks otherwise (Node rows, evidence §Final+GREEN reception and §Final+GREEN r2 reception) | `worker_threads-keepalive.fault.test.ts` rows 1–2 and the failure rows (kernel peer death and three failed starts, each unlistened-uncaught / unlistened-fatal / listened; real ProcessManager and realm error trap; only the absent DOM Worker and the browser's microtask exception report are substituted); browser-unit `worker-handle-keepalive` `failed-start-*` vs live Node on Node's timing-independent rows (every row but `wexit` in order, `wexit` where Node always prints it) | → I2, ADR-0446 |
...
re-cut: 2026-09-25 — widened: the peer-death row covers a failed start whose `'error'` has no listener (Node order, release, no hang) with fault-test and browser-unit carriers (Final+GREEN r1 blocker, evidence §Final+GREEN reception) — trace: none
re-cut: 2026-09-25 — peer-death row: an unlistened peer-death `'error'` joins the failure rows (uncaught before `'exit'`); Node's `'exit'` placement recorded as timing-dependent (rifty pins an unloaded Node run's order in the fault test) and the browser-unit failed-start carrier compares Node's timing-independent rows (Final+GREEN r2 blockers, evidence §Final+GREEN r2 reception) — trace: none
