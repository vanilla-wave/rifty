...
| peer death (fault-classes §Boundary: dedicated Worker) × a kernel peer closing, or the DOM Worker boundary refusing the spawn; a failed start (refused spawn, `data:` URL gap, same-realm load failure) whose `'error'` has no listener | `'error'` then `'exit'` 1 (after the microtasks the `'error'` handling queued), and every hold the Worker took is released; the parent never hangs. An unlistened `'error'` is the parent's uncaught exception first; `'exit'` 1 follows unless it ended the parent (Node rows, evidence §Final+GREEN reception) | `worker_threads-keepalive.fault.test.ts` rows 1–2 and the failed-start rows (real ProcessManager and realm error trap; only the absent DOM Worker and the browser's microtask exception report are substituted); browser-unit `worker-handle-keepalive` `failed-start-*` vs live Node | → I2, ADR-0446 |
