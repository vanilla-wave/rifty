...
advanced fork preserves Date, Map, `[undefined]`, Uint8Array, BigInt and cycles
...
1. `fork(..., { serialization: 'advanced' })` starts one physical Worker.
   Child→parent and parent→child preserve Date, Map, `[undefined]`,
   Uint8Array, BigInt and a cyclic plain object by type/value/identity. → I4
2. Invalid sends reject synchronously with Node's error class/code; a later
   ...
   ends disconnected and the child exits 0. → I4, ADR-0448
...
4. A Buffer nested in an advanced payload and accessor-bearing payloads
   throw named `NotImplementedError`, not a silently changed type or
   getter evaluation. → ADR-0448
...
| `provenance-lie` | nested Buffer/accessor is named loud rather than losing brand or invoking getters | advanced IPC fault test → ADR-0448 |
| `corrupt-input` | unknown launch IPC discriminator fails before guest entry | node-entry bootstrap test → ADR-0448 |

## Out of scope

- Node's V8-specific Buffer wire representation, handles, callbacks/options,
