...
sources: [docs/backlog/runtime-js/reference/vitest-run-in-browser-evidence.md, docs/backlog/runtime-js/reference/child-process-advanced-ipc-serialization-evidence.md, docs/backlog/runtime-js/reference/advanced-ipc-clone-fidelity-evidence.md, docs/backlog/runtime-js/reference/advanced-ipc-prototype-fidelity-evidence.md, docs/backlog/runtime-js/reference/advanced-ipc-arraybuffer-alias-evidence.md, docs/adr/runtime-js/0448-advanced-fork-ipc-over-the-existing-process-channel.md, docs/adr/runtime-js/0454-reject-unsupported-shared-backed-advanced-ipc-views.md, docs/adr/runtime-js/0457-inspect-advanced-ipc-internal-brands-after-prototype-changes.md, docs/adr/runtime-js/0458-reject-explicit-arraybuffer-view-aliases-in-advanced-ipc.md]
...
in both directions. Top-level undefined/function/BigInt and nested function reject
...
ADR-0458 keeps an explicit ArrayBuffer/view alias graph loud until its V8 wire
behavior can be reproduced; standalone Uint8Array type/bytes stay admitted.
...
2. Invalid sends, including top-level BigInt, reject synchronously with Node's error class/code; a later
...
6. A graph containing both an ArrayBuffer and its view throws `NotImplementedError('child_process.serialization.advanced.arraybuffer-view-alias')` before posting; standalone Uint8Array remains admitted by type/bytes. → ADR-0458
...
5. `reference/advanced-ipc-arraybuffer-alias-oracle.mjs` runs the same graph in a physical Node fork and rifty Worker; the fault test pins the resulting named ceiling. → ADR-0458

## Fault matrix

| Fault | Boundary and outcome | Proof |
|---|---|---|
| `corrupt-input` | top-level undefined/function/BigInt and nested function reject synchronously; next valid message still arrives | physical parity case → ADR-0448 |
...
| `provenance-lie` | replaced prototypes cannot hide raw SAB or Map/Set entries; unsupported views stay named loud | advanced IPC fault test and physical Node oracle → ADR-0457 |
| `provenance-lie` | explicit ArrayBuffer/view alias cannot retain shared mutations as a claimed successful IPC message | physical Node/Rifty oracle and advanced IPC fault test → ADR-0458 |

## Out of scope

- Node's V8-specific Buffer wire representation, plain-object accessors, handles, callbacks/options,
  SAB-backed views, explicit ArrayBuffer/view aliases, channel `ref()`/`unref()` and non-structured-cloneable host objects remain
  loud gaps in compat; no JSON fallback.

## Decisions

- re-cut: 2026-09-23 — ADR-0458 names explicit ArrayBuffer/view alias graph loud after physical Node/Rifty mutation RED; Vitest default-forks census has no such graph — trace: ADR-0458
- re-cut: 2026-09-23 — ADR-0457 class-kill for changed-prototype internal brands after observed silent SAB/Buffer escape — trace: ADR-0457
- re-cut: 2026-09-23 — physical Node fork RED added top-level BigInt to ADR-0448 invalid-send row — trace: ADR-0448
