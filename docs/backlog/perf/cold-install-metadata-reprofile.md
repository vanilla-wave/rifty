...
- Decision rule: bytes-dominant → favor `bundled-popular-subgraph-metadata` (or stop at corgi); RTT-dominant → favor `server-side-closure-resolver`; connection-dominant → the h3 lever in `perf/eddy-http3-cold-validation` (ex `install-transport-tuning`; its preconnect half shipped in ADR-0186).
