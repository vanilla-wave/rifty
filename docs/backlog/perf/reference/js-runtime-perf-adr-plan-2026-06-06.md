...
- 1-2 never-executed warm workers; identity bound only at claim/init time (preserves ADR-0011 "own realm per process"). Record the **conditional/measured-need gate** (measured-need gate — CLAUDE.md "inflections are not stops"): design recorded now, build conditioned on a measured spawn spike, separate PR. No decision subagent. *(Auditor corrected the verdict's proposed "0083" → 0090.)*
...
- **DECISION-SUBAGENT-REQUIRED ✅** — reconsidering a recorded decision other work depends on (CLAUDE.md); the subagent reads ADR-0048/0017/0055 + draft 0060 and produces ADR-0093.
...
## D. Code marker referencing an EXISTING reversible-decision (backlog) entry
...
## E. NO ADR / NO backlog entry needed (internal, byte-identical, small)
...
5. **process/npm:** 0088 (npm concurrency) and 0089 (lazy builtins) are independent; 0090 (pre-warm) is **conditionally built** — record design now, gate build on a measured spawn spike (inflection gate, CLAUDE.md). Q-322 independent.
