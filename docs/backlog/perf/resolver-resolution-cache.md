...
## Options / Next
Decision: memoize `resolveSpecifierToFile` (NOT the resolve() boundary — readResolved must re-read source fresh) keyed `${esm}\0${fromFile}\0${specifier}` → file-id; full-clear on ANY invalidate; NEVER cache not-found (guest fs.writeFileSync / npm-install-then-require writes to shared VFS without firing invalidate → poisoned absence); never cache the ERR_PACKAGE_PATH_NOT_EXPORTED throw path. Loader owns the result Map (mirrors transformCache) so Resolver interface stays unchanged. Record here; TODO(backlog: perf/resolution-cache) marker at resolver.ts (resolveSpecifierToFile ~line 167).
## Reversibility
