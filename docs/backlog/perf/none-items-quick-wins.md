...
---
## Context
Reversible, internal, byte-identical small wins (CLAUDE.md "always reversible" / NONE class). Ship immediately, one PR per concern; no ADR/backlog item. Spans io/vfs/module-loader/npm-client/net/runtime-js. ascii-mask is a parity-driven correctness fix (not a design fork) and ships in its own PR with a FAILING parity case written first.
## Options / Next
...
- ascii-mask: ascii decode `& 0x7f` 7-bit mask (buffer-codec.ts:72-74) — toString('ascii') diverges from Node for bytes ≥0x80; OWN PR, FAILING parity case FIRST; does not contradict ADR-0030.
## Reversibility
