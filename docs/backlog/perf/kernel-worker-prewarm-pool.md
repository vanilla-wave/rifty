...
## Options / Next
1-2 never-executed warm workers; identity bound only at claim/init time (preserves ADR-0011 "own realm per process"). Record the conditional/measured-need gate (measured-need gate — CLAUDE.md "inflections are not stops"): design recorded now, build conditioned on a measured spawn spike, separate PR. Per-spawn env diff (Q-2026-06-06-322) is the independent companion lever.
## Reversibility
