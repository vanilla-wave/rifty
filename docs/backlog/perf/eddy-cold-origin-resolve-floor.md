...
4. **Bundle store = disk-only, REPLACING the in-process RAM LRU** — files at `bundles/<npmClientVersion>/<closureHash>`, served via stream; the kernel page cache IS the hot tier (free, system-wide, byte-bounded by the OS). Kills the RSS risk as a class (immutable LRU today caps ENTRIES: 256 × 3–7MB ≈ up to ~1.5GB in-process), restart-warmth is a free side effect (version prefix handles deploy bumps; stale dirs GC'd), size-capped GC by atime. Corrupt file is safe: the client verifies every tarball against the manifest → decline → POST recompute overwrites. In RAM stay only the small things: mutable `depSetKey→closureHash` links (TTL) + the shared packument cache (TTL shorter than any persistence pays for). NOT justified by speed — steady-state RAM≈disk (POST bottleneck = TLS+transfer) — justified by memory-bounding with restart-warmth as bonus.
