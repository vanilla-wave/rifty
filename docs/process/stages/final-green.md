...
- Pass → the slice has landed. Inside a goal `rechart.md` records `re-chart
  ...
  slice's `BASE`; a unit without a goal records nothing: the runner checks
  the binding, deletes the doc in a last bookkeeping commit (delete on done)
  and merges (`REV-8`, `DEC-3`); a unit with no doc just merges.
