...
- Pass → `ready-verdict: <date> — Contract+RED @ <sha>` as the first
  ...
  (`../rules/pr.md` `PR-3`).
- Blocker → `contract-red: <date> — blocker @ <sha>`; one batch fix; one verify
  pass (budget `RDY-9`). A 2nd consecutive blocker → `../rules/stops.md`
  `STOP-5`: the contract is wrong — re-cut (`STOP-4`), never another round.
