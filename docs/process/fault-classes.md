...
| SAB sync-RPC ring (child caller ↔ parent dispatcher) | shared memory: no loss/dup/reorder while both realms live; single-request-at-a-time by contract, enforced only by guards at write time | second caller/consumer on one ring (double-consume reads len=0; interleaved writeRequest trips the guard) · abandoned exchange wedges the slot permanently — every later call fails with "previous reply is unread" · responder death parks the blocked caller forever (no timeout by default) · oversize reply degrades to an in-band error reply | transport loss, duplicate delivery, reorder |
...
| Checkpoints | Contract+RED before implementation; Final+GREEN on one checked SHA; blocker → redesign/re-cut, never checkpoint three |
