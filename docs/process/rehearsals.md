...
:102-105, :111-117, §Report :143-145; `docs/process/README.md:49-51`;
`rifty-goal/SKILL.md:20, 25-28`; `rules/pr.md` `PR-3` :20-21;
`epics/TEMPLATE.md:30, 62-68`; `artifacts/map.md:9`; `RDY-6` 4 :125;
AGENTS.md:31
("epic missing tier/Invariants → fit it yourself"); unchanged: `RDY-1`,
`STOP-1a` (stops.md:15-18), `STOP-4` (stops.md:23-27), `rifty-to-backlog`
§4 :43 / §5 :60-61, `stages/pickup.md`, `implement.md:18`.
...
|---|---|---|---|---|
| R1 | epic · forks closed · user present / PR #333 `epics/ai-agent-mode-and-bench` | owner: "воскресить #111", then hand the goal off in one command | refine §4:56-58 → fit.md 4 (Invariants, tier) → 8 (one final check on the whole set) → 9 `ready` + report → on its branch, packaging `PR-3` → README:39 hand-off → `rifty-goal`:21 PICKUP | **pass** / **fail**: refine §4 "ends with preparation" + §3.6 vs TEMPLATE:30 dual owner → Invariants empty; `rifty-goal` had no row for a draft goal dir (silence); FIT scheduled for a session without the user (fit.md:6-7), PR merge as a stage boundary (`PR-3`), "immediately runnable" (README:101) not met. Tally before 2·0·4+2·~12·2·2; after 1·0·4·~12·1·1 |
| R2 | epic · one user question unanswered / `no-coi-sandbox-tier` FIT 2026-08-28 | owner: "не знаю пока", wants the rest to proceed | refine §3 grill → no answer → fit.md:24-26 goal stays `draft` (step 9 does not flip); §4: FIT resumes on the answer — in-session (§3) or at hand-off: `rifty-goal`:20 FIT, :25-28 ask the recorded question first (`STOP-1a` → stops.md:18), continue on the goal PR's branch; PR packaging driver's (`PR-3`) | **pass** / **fail** before: README §Epic fit :114-116 let the question ride as `owner: user` fog in a `ready` goal → the autonomous run stops at `STOP-1a` mid-way; `rifty-goal` table had no draft-goal row |
...
| R5 | standalone item · same day / `playground/react-vite-starter` (created + Contract+RED 2026-09-02, PR #300) | maintainer: build the starter now | refine → §4:65-66 item `draft → ready` at PICKUP (`RDY-1`) → README:48 → pickup.md 2 → Contract+RED → IMPLEMENT, one PR (implement.md:18) | **pass** / pass; backfire clean — §4:56 says "An epic" |
...
README:41-42 "never return control between stages", fit.md:6-7 "the user is
