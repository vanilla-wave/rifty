...
Authorities: `rifty-refine/SKILL.md` §3.6, §4 (lines 51, 56-61, 64-65);
`stages/fit.md:5-8`; `docs/backlog/README.md` §Epic fit :102-105, §Report
:143-145; `docs/process/README.md:49-50`; `rifty-goal/SKILL.md:20, 25-27`;
`rules/pr.md` `PR-3` :20-21; `epics/TEMPLATE.md:30`; unchanged: AGENTS.md:31
("epic missing tier/Invariants → fit it yourself"), `RDY-1`, `RDY-6`,
`STOP-1a` (stops.md:15-18), `rifty-to-backlog` §4 :43 / §5 :60-61,
`stages/pickup.md`, `implement.md:18`.
...
|---|---|---|---|---|
| R1 | epic · forks closed · user present / PR #333 `epics/ai-agent-mode-and-bench` | owner: "воскресить #111", then hand the goal off in one command | refine §4:56-58 → fit.md 4 (Invariants, tier) → 8 (one final check on the whole set) → 9 `ready` + report → same PR (`PR-3`) → README:39 hand-off → `rifty-goal`:21 PICKUP | **pass** / **fail**: refine §4 "ends with preparation" + §3.6 vs TEMPLATE:30 dual owner → Invariants empty; `rifty-goal` had no row for a draft goal dir (silence); FIT scheduled for a session without the user (fit.md:5), PR merge as a stage boundary (`PR-3`), "immediately runnable" (README:101) not met. Tally before 2·0·4+2·~12·2·2; after 1·0·4·~12·1·1 |
| R2 | epic · one user fork open / `no-coi-sandbox-tier` FIT 2026-08-28 | owner: "не знаю пока", wants the question kept and the rest to proceed | refine §3 grill → `RDY-6` 4 dependent waits → §4:59-61 goal stays `draft`, fog `owner: user` + why (README:111-116) → PR packaging driver's (`PR-3`) → hand-off: `rifty-goal`:20 FIT, :25-27 ask first (`STOP-1a` → stops.md:18), continue on the same branch; user answers in-session → §4:56 FIT continues | **pass** / silence: `rifty-goal` table listed only "No goal dir" |
| R3 | legacy ready single-file epic / `epics/embeddable-dev-loop.md` (no Invariants) | owner hands off "run embeddable-dev-loop" | `rifty-goal`:20 legacy → fit.md:7-8 FIT in the hand-off session, step 3 asks the user present → 9 `ready` → PICKUP | **pass** / pass (banked G); backfire re-rehearsed clean — no refine session required |
| R4 | agent-only epic capture — **invented** (pole origin=nobody asked × size=epic; captures on record are items) | post-merge audit agent finds a multi-item outcome | `rifty-to-backlog` §1-4 draft, §5:60-61 no interview → refine not entered (Preconditions:12) → own docs-only PR only with no branch (§4:43, `PR-2`) → user brings it → README:47 refine → FIT then | **pass** / pass; the fix does not reach it |
| R5 | standalone item · same day / `playground/react-vite-starter` (created + Contract+RED 2026-09-02, PR #300) | maintainer: build the starter now | refine → §4:64-65 item `draft → ready` at PICKUP (`RDY-1`) → README:48 → pickup.md 2 → Contract+RED → IMPLEMENT, one PR (implement.md:18) | **pass** / pass; backfire clean — §4:56 says "An epic" |
| R6 | mid-slice discovery / banked M `process.ts:659` | agent mid-slice | `rifty-to-backlog` §4:42-43 rides the unit branch (`PR-2`) | pass on routing; ceremony fail carried (banked M) |
| R7 | user "just file it" / banked O `runtime-js/node-entry-runtime-binding-variant-coverage` (epic variant: same clause) | user: record the gap, decide later | refine §3 "just file it → Formalize" → §4:59 goal stays `draft` → later `rifty-goal`:20 | **pass** / pass; backfire clean — FIT not forced |
| R8 | hand-off right after FIT, refine PR unmerged / PR #333 counterfactual "теперь запускай" | owner: start the run without a merge round-trip | refine §4 → `ready` on branch → README:39 → `rifty-goal`:21 PICKUP → branch/PR choice: `PR-3` "one draft PR per goal by default; combine or split" | **pass** with named silence (which branch: driver's, principle `PR-3`) / same |
| R9 | the process text itself / this diff | maintainer: fix the text, rehearse, one PR | README:48 authorized change, no minted draft (`PR-2`) → `RDY-8` docs checks + Final+GREEN → `pr:check` doc lanes | pass on routing; ceremony fail carried (banked B). Observed: `pr:check` 19/20 in a fresh worktree (`check:esbuild-legacy-retirement`: build outputs missing), 20/20 docs-only after `pnpm build:libs` |
...
README:41-42 "never return control between stages", fit.md:5 "the user is
present", README:101 "immediately runnable". R2 before: silence on the driver
...
R6/R9 ceremony (banked constraint: cost keyed to unit shape) or the eight
legacy `ready` epics without Invariants (by design: FIT at hand-off, R3).

Backfire: R2 (open fork must stay `draft`) — §4:59 names it; R3 (legacy
without refine) — fit.md:7-8; R4 (agent-only never interviews) — refine
Preconditions:12 + `rifty-to-backlog`:60-61; R5 (items never FIT) — §4:56
"An epic"; R7 (just-file) — §4:59. All re-rehearsed clean. Residual silence:
