...
- The **REQUEST side** (IncomingMessage pull-stream contract: `read(n)`/`'readable'`/`'end'`/`'error'`/`'off'`/`'destroy'`, plus `headers`/`socket.remoteAddress`/`url`/`method`) is already satisfied **AS-IS** by `packages/net/src/http/request.ts:69-83` over `@riftydev/io` Readable — no change needed, same as the express@4 precedent.
...
3. **UNIT (Q-103 pipe-sink):** a focused unit test that `Readable.fromWeb(webStream).pipe(res)` terminates and produces the right bytes IF `@riftydev/io` `Readable.pipe` accepts the duck shape; if not, an xfail/skip documenting the deferred gap.
...
- **Parity runner CANNOT host this feature:** `tools/node-parity-runner` only loads `@riftydev/runtime-js/loader` and does NOT register `node:http` (the `node:http` builtin lives in `@riftydev/net`, which the runner never imports — see `parse-url.case.ts:9-13` stating exactly this). So the CLAUDE.md 'parity is the gold standard' default is architecturally unavailable here; the primary tests are `packages/net` UNIT tests asserting Node-faithful behavior, cross-checked against a one-shot manual Node reference where cheap. This is a justified deviation, not a shortcut.
