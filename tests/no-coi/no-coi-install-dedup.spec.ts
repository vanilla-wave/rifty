  ...
  const result = await page.evaluate(
    async ({ root, nativeReplicaProbeSource }) => {
      ...
      try {
        ...
        observeNativeReplicaWrites(records => {
          for (const record of records) {
            if (record.kind === 'file' && record.path === '/dedup/node_modules/nanoid/index.js') globalThis.__installCounts.writes++;
            if (record.kind === 'dir' && record.path === '/dedup/node_modules/nanoid') globalThis.__installCounts.mkdir++;
          }
        });
      ...
      `);
        ...
        await sandbox.fs.writeFile(
          '/dedup/node_modules/nanoid/index.js',
          original.replace(/./, '!'),
        );
        await sandbox.toolchain.install({ cwd: '/dedup', registryUrl: '/npm-registry' });
        ...
        const quotaInjected = await sandbox.runtime.eval(`
        ${nativeReplicaProbeSource}
        globalThis.__quota = true;
        observeNativeReplicaWrites(records => {
          if (globalThis.__quota && records.some(record => record.kind === 'file' && record.path === '/dedup/node_modules/nanoid/index.js')) throw new DOMException('dedup quota', 'QuotaExceededError');
        });
      `);
        if (!quotaInjected.ok) throw new Error('native quota injection failed');
        let rejected = false;
        try {
          await sandbox.toolchain.install({ cwd: '/dedup', registryUrl: '/npm-registry' });
        } catch {
