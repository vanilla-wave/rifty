...
test('real Vite HMR survives explicit wedge restart and reports the dirty boundary — designed RED', async ({
  ...
  try {
    ...
    await page.evaluate(async () => {
      ...
      if (typeof sandbox.onLifecycle !== 'function') {
        throw new Error('ToolchainSandbox.onLifecycle is missing');
      }
      sandbox.onLifecycle?.((event) => {
        (Reflect.get(globalThis, '__riftyLifecycleEvents') as LifecycleEvent[]).push(event);
      });
    });
    ...
    const report = await page.evaluate(async () => {
      ...
      return { restarted, events: Reflect.get(globalThis, '__riftyLifecycleEvents') };
    });
    ...
    const clean = await page.evaluate(async () => {
      const sandbox = Reflect.get(globalThis, '__riftyDevSandbox') as DevSandbox;
      const iframe = document.querySelector('#dev-preview') as HTMLIFrameElement;
      return sandbox.restart?.({ preview: iframe });
    });
    expect(clean).toMatchObject({ unflushedWrites: false });
  } finally {
    await disposeSandbox(page);
  }
});

test('actual Worker close emits one death event and settles pending work — designed RED', async ({
  page,
}) => {
  await bootSandbox(page);
  try {
    const outcome = await page.evaluate(async () => {
      ...
      sandbox.onLifecycle?.((event) => events.push(event));
    ...
    expect(outcome).toEqual({
      events: [{ type: 'worker-death', unflushedWrites: false }],
