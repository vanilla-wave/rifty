...
const HEADERS = 'apps/landing/_headers';
...
describe('landing static site', () => {
  it('publishes the rifty.dev landing page with playground and GitHub exits', () => {
    expect(existsSync(INDEX)).toBe(true);

    const html = readFileSync(INDEX, 'utf8');
    expect(html).toContain('<title>rifty — Node.js stack in the browser</title>');
    expect(html).toContain('<link rel="canonical" href="https://rifty.dev/" />');
    expect(html).toContain('href="https://play.rifty.dev/"');
    expect(html).toContain('href="https://github.com/vanilla-wave/rifty"');
    expect(html).toContain('Open, self-hostable browser runtime infrastructure');
  });
  ...
  it('deploys the landing as its own Netlify project', () => {
    ...
    expect(workflow).toContain('--dir="$GITHUB_WORKSPACE/apps/landing"');
