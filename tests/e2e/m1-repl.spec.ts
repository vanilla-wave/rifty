...
async function terminalBuffer(term: Locator): Promise<string> {
  return ((await term.getAttribute('data-terminal-buffer')) ?? '').replace(ANSI_SGR, '');
}
...
test.describe('M1 — JS Execution', () => {
  test('terminal evaluates expressions', async ({ page }) => {
    ...
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('2');
  });
  ...
  test('Run button executes editor source and streams stdout', async ({ page }) => {
    ...
    await expect.poll(() => terminalBuffer(term), { timeout: 10_000 }).toContain('worker alive');
  });
  ...
  test('errors render with traceback in red (stderr stream)', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('boom');
  });
  ...
  test('console.log goes to stdout, console.error goes to stderr', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('OUT-A');
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('ERR-B');
  });

  test('.reset respawns the worker', async ({ page }) => {
    await page.goto('/');
    const term = page.locator('[data-testid="terminal"]');
    await expect.poll(() => terminalBuffer(term), { timeout: 10_000 }).toContain('[worker ready]');
    await term.click();
    await page.keyboard.type('globalThis.__mark = 17');
    await page.keyboard.press('Enter');
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('17');
    ...
    await expect
      .poll(() => terminalBuffer(term), { timeout: 5000 })
      .toContain('[worker exited: reset]');
    await page.keyboard.type('typeof globalThis.__mark');
    await page.keyboard.press('Enter');
    await expect.poll(() => terminalBuffer(term), { timeout: 5000 }).toContain('undefined');
  });
