...
test('URL entry transfers and one-shot consumes an opaque capability port', async ({ page }) => {
  ...
  const result = await page.evaluate(
    async ({ entryUrl, hostAssetsUrl, kernelUrl }) => {
      const kernel = await import(kernelUrl);
      const hostAssets = await import(hostAssetsUrl);
