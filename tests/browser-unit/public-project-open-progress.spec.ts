...
for (const legacy of [false, true])
  test(`public Workbench reports actual first-open persistence and clears it at settlement${legacy ? ' with legacy layout notice' : ''}`, async ({
    page,
  }) => {
    ...
    const result = await page.evaluate(
      async ({ snapshot, legacy }) => {
        ...
        const namespace = `pr323-progress-${crypto.randomUUID()}`;
        if (legacy) {
          let root = await (await navigator.storage.getDirectory()).getDirectoryHandle(namespace, {
            create: true,
          });
          for (const part of ['.rifty', 'workbench', 'v1'])
            root = await root.getDirectoryHandle(part, { create: true });
          const writer = await (
            await root.getFileHandle('old.txt', { create: true })
          ).createWritable();
        ...
        const unsubscribe = workbench.health.subscribe((health) => {
          const progress = (
            health as unknown as {
              projectOpen?: {
                projectId: string;
                persistence?: { persisted: number; total: number };
              };
            }
          ).projectOpen;
    ...
    if (legacy)
      expect(result.settled.issues.some((issue) => issue.scope === 'storage-layout')).toBe(true);
