...
import { capturePlaygroundLegacyWorkspacePrefix } from '../../../packages/workbench/src/workbench/internal/playground-boot-config.ts';
...
export async function seedLegacyWorkspace(input: LegacyWorkspaceSeed): Promise<void> {
  ...
  const prefix = capturePlaygroundLegacyWorkspacePrefix({
    getItem: (key) => (key === 'rifty.workspaceId' ? input.workspaceId : null),
  });
  if (prefix === undefined) throw new Error('Legacy workspace seed has no storage prefix');
