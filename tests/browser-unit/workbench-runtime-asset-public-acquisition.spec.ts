...
const expectedVite7CapabilityKeys = [
...
test('public Vite 7 cold open attests assets before its real child is published', async ({
  ...
  const result = await page.evaluate(
    async ({ fixtureUrl, expectedCapabilityKeys }) => {
      ...
      try {
        ...
        if (ready?.phase !== 'ready')
          throw new Error('project open resolved without ready progress');
        ...
        if (
          ...
          proof.assetSize <= 0 ||
          JSON.stringify(proof.capabilityKeys) !== JSON.stringify(expectedCapabilityKeys)
        ) {
          throw new Error(
    ...
    { fixtureUrl: childEntryUrl, expectedCapabilityKeys: expectedVite7CapabilityKeys },
  );
  ...
  expect(result.proof).toMatchObject({
    capabilityKeys: expectedVite7CapabilityKeys,
...
test('a non-empty runtime-asset capability stays outside every ordinary guest projection', async ({
  ...
  for (const privateToken of [
    VITE_CONFIG_TEMP_CACHE_CAPABILITY,
    'rifty.vite-config-temp-cache/v1',
