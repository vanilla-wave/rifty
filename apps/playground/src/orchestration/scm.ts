...
export function createScm<O extends FileReadOwnerLike>(deps: ScmDeps<O>): Scm<O> {
  ...
  function assertOwnerStillCurrent(
    owner: O,
    root: string,
    snapshotPort: unknown,
    message: string,
  ): void {
    ...
    if (
      currentOwner !== owner ||
      currentOwner.snapshotPort !== snapshotPort ||
  ...
  async function readGitOriginalText(input: {
    ...
    try {
      ...
      assertOwnerStillCurrent(
        owner,
        root,
        snapshotPort,
        `workspace owner changed while reading ${input.ref}:${relative}`,
      );
  ...
  async function headBlobExistsForCurrentStatus(
    ...
    try {
      ...
      assertOwnerStillCurrent(
        ...
        root,
        snapshotPort,
        `workspace owner changed while comparing ${relative} with HEAD`,
      );
  ...
  async function openWorkingHeadCompare(path: string): Promise<void> {
    try {
      ...
      const hasOriginal = await headBlobExistsForCurrentStatus(
        owner,
        path,
        relative,
        root,
  ...
  async function openScmResourceDiff(row: ScmResourceRow): Promise<void> {
    ...
    try {
      ...
      assertOwnerStillCurrent(
        ...
        root,
        snapshotPort,
        `workspace owner changed while opening ${relative}`,
      );
