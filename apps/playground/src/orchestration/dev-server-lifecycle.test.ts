...
const NODE_CLI_PRESET = { id: 'node-cli', setup: 'instant' } as unknown as Preset;
...
class Harness {
  ...
  starterPreset: Preset = VITE_PRESET;
  private nextId = 10;
  ...
  deps(): DevServerLifecycleDeps<Session> {
    return {
      ...
      activeStarterPreset: () => this.starterPreset,
      templateForPreset: (preset) => (preset === NODE_CLI_PRESET ? NODE_CLI_SPEC : VITE_SPEC),
...
function setup(prep?: (h: Harness) => void): Ctx {
  ...
  // App binds this via a one-line createEffect on the owner signal; the swap
  // semantics under test are the module's own attachOwner contract.
...
describe('boot/stop/restart sequencing', () => {
  it('startSession claims the session, paints a fresh console + banner, runs the preset boot lines', async () => {
    ...
    const boot = dev.startSession('t1', gen, VITE_PRESET);
  ...
  it('never boots into a hidden/closed target — falls back to a visible session', async () => {
    ...
    const gen = dev.nextGeneration();
    const boot = dev.startSession('ghost', gen, VITE_PRESET);
    await until(() => h.bootRuns.length === 1);
  ...
  it('honors bootLinesOverride over the preset lines', async () => {
    const { h, dev, dispose } = setup();
    const gen = dev.nextGeneration();
    const boot = dev.startSession('t1', gen, VITE_PRESET, ['npm run dev -- --port 4000']);
    await until(() => h.bootRuns.length === 1);
  ...
  it('waits for node-cli presets as terminal commands, not preview servers', async () => {
    ...
    h.session('t1', 'running');
    const gen = dev.nextGeneration();
    const boot = dev.startSession('t1', gen, NODE_CLI_PRESET);
    await tick(120);
  ...
  it('restart stops the tracked session and boots the ACTIVE STARTER preset (never a stale pick)', async () => {
    ...
    dev.beginBoot('t1');
    h.owner.emitDevServer({ status: 'running', sid: 't1' });
    h.starterPreset = { id: 'switched-starter', setup: 'instant' } as unknown as Preset;
