...
export const VITE8_TEMPLATE: ViteProjectSpec = {
  ...
  server: {
    appType: 'spa',
    strictPort: true,
    // Vite 8 dep pre-bundling drives Rolldown's WASI bundler, upstream-broken
    // for this path (ADR-0168 corrected) — stays off regardless of the real
    // esbuild bridge (ADR-0192 covers esbuild, not Rolldown).
