...
export function createEditorHostCore(props: EditorHostProps, host: EditorHostSurface) {
  ...
  let ownerUpdate: monaco.editor.ITextModel | undefined;
  ...
  const api: EditorApi = {
    ...
    openTextDiff,
    flushPendingWrites,
    applyOwnerBytes(path, bytes) {
      const model = models.get(path);
      const write = writeStates.get(path);
      if (
        !model ||
        readOnlyPaths.has(path) ||
        (write && write.publishedGeneration < write.generation)
      )
        return false;
      const text = dec.decode(bytes);
      if (model.getValue() === text) return true;
      const previous = ownerUpdate;
      ownerUpdate = model;
      try {
        model.setValue(text);
      } finally {
