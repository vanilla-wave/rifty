...
function expectImportRejectionBeforeAnyFilesystemEffect(
  ...
  expect(() => preparePlaygroundArchiveV1Import(fs, PROJECT_ROOT, json, limits)).toThrow();
  expect(mkdir).not.toHaveBeenCalled();
  expect(remove).not.toHaveBeenCalled();
  expect(writeFile).not.toHaveBeenCalled();
  expect(rename).not.toHaveBeenCalled();
...
describe('Playground archive v1 finite budgets', () => {
  ...
  it('accepts import exactly at every budget and counts JSON in UTF-16 code units', () => {
    ...
    expect(prepared.decodedFiles()).toEqual([
      { path: '\ud83d\ude00.txt', bytes: encoder.encode('12') },
      { path: 'b.txt', bytes: encoder.encode('345') },
    ]);
...
describe('Playground archive v1 export contract', () => {
  ...
  // Fault class: lossy-snapshot + sibling-drift. Git and nested `.rifty` are
  // observable workspace bytes; only derived trees and root authority are omitted.
  it('exports observable Git and nested .rifty while excluding derived trees and root authority', () => {
    const fs = new MemoryFsSync();
...
describe('Playground archive v1 strict import boundary', () => {
  const invalidLastCases: readonly InvalidLastCase[] = [
    ...
    },
    {
      ...
      json: archive([
        file('valid-first.txt', 'valid'),
        file('.rifty/invalid-last.json', 'invalid'),
      ]),
    },
    {
      name: 'collision',
  ...
  it('decodes an empty archive without mutating the project', () => {
    const fs = new MemoryFsSync();
    write(fs, `${PROJECT_ROOT}/old.txt`, 'old');
    const before = snapshotWholeFs(fs);

    const prepared = preparePlaygroundArchiveV1Import(fs, PROJECT_ROOT, archive([]));

    expect(prepared.decodedFiles()).toEqual([]);
    expect(snapshotWholeFs(fs)).toEqual(before);
  });

  it('decodes a zero-byte file and allows non-reserved lookalike segments', () => {
    const fs = new MemoryFsSync();
    ...
    const prepared = preparePlaygroundArchiveV1Import(
      ...
      archive([
        ...
        file('nested/.rifty/settings.json', '{}'),
      ]),
    );

    expect(prepared.decodedFiles()).toEqual([
      { path: 'empty.bin', bytes: new Uint8Array() },
      { path: '.git/HEAD', bytes: encoder.encode('ref: refs/heads/main\n') },
      { path: 'distillery/source.ts', bytes: encoder.encode('kept') },
      { path: '.github/workflows/check.yml', bytes: encoder.encode('kept') },
      { path: 'nested/.rifty/settings.json', bytes: encoder.encode('{}') },
    ]);
  });
