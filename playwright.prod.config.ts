...
/**
 * PRODUCTION-build acceptance config. The default `playwright.config.ts` runs every
 ...
 * `tests/e2e-prod/` smoke and full-journey specs against that real artifact.
 */
...
export default defineConfig({
  ...
  // Fail-fast on CI: the prod suite is serial against a fresh build —
